...
unweighted_re_04month = read_tsv("unweighted-distance-matrix-04month.tsv")
weighted_re_04month = read_tsv("weighted-distance-matrix-04month.tsv")
unweighted_re_412month = read_tsv("unweighted-distance-matrix-412months.tsv")
weighted_re_412month = read_tsv("weighted-distance-matrix-412months.tsv")
...
seq_depth = read_tsv("per-sample-fastq-counts-all.txt")
metadata_re = metadata %>%
  left_join(seq_depth, by = c("SampleID" = "sample ID")) %>% 
...
metadata_re_312year = metadata %>% 
  right_join(unweighted_re_312year, by = c("SampleID" = "...1")) %>% 
               dplyr::select(1:45)
metadata_re_04month = metadata %>% 
  right_join(unweighted_re_04month, by = c("SampleID" = "...1")) %>% 
               dplyr::select(1:45)
metadata_re_412month = metadata %>% 
  right_join(unweighted_re_412month, by = c("SampleID" = "...1")) %>% 
               dplyr::select(1:45)
...
unweighted_re_04month_dist = as.dist(unweighted_re_04month[,2:715])
weighted_re_04month_dist = as.dist(weighted_re_04month[,2:715])
unweighted_re_412month_dist = as.dist(unweighted_re_412month[,2:748])
weighted_re_412month_dist = as.dist(weighted_re_412month[,2:748])
...
### Per study sequencing depth plot

```{r seq_depth}
summarize_seq_depth = metadata_re %>% 
  group_by(Study) %>% 
  summarise(mean = mean(`forward sequence count`), 
            median = median(`forward sequence count`), 
            max = max(`forward sequence count`), 
            min = min(`forward sequence count`),
            std_dev = sd(`forward sequence count`))
seq_depth_plot = ggboxplot(metadata_re, x = "Study", 
                  y = "forward sequence count", color = "Study", 
...
seq_depth_plot = ggpar(seq_depth_plot, legend = "right") + 
  rremove("xlab") + 
...
adonis2(unweighted_re_dist ~ Study, data=metadata_re, by = "margin", permutations = 499, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(unweighted_re_dist ~ Sequencing, data=metadata_re, by = "margin", permutations = perm, parallel = 4)

adonis2(unweighted_re_dist ~ Primer_set, data=metadata_re, by = "margin", permutations = 499, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(unweighted_re_dist ~ Primer_set, data=metadata_re, by = "margin", permutations = perm, parallel = 4)

adonis2(unweighted_re_dist ~ `forward sequence count`, data=metadata_re, by = "margin", permutations = 499, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(unweighted_re_dist ~ `forward sequence count`, data=metadata_re, by = "margin", permutations = perm, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(unweighted_re_dist ~ Sequencing + Primer_set + `forward sequence count`, data=metadata_re, by = "margin", permutations = perm, parallel = 4)
...
anova(betadisper(unweighted_re_dist, group = metadata_re$Sequencing))
anova(betadisper(unweighted_re_dist, group = metadata_re$Primer_set))
anova(betadisper(unweighted_re_dist, group = metadata_re$`forward sequence count`))
...
perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re_04month, Study)
adonis2(unweighted_re_04month_dist ~ Race + Ethnicity, data = metadata_re_04month, by = "margin", permutations = perm, parallel = 4)
anova(betadisper(unweighted_re_04month_dist, group = metadata_re_04month$Race))
anova(betadisper(unweighted_re_04month_dist, group = metadata_re_04month$Ethnicity))

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re_412month, Study)
adonis2(unweighted_re_412month_dist ~ Race + Ethnicity, data = metadata_re_412month, by = "margin", permutations = perm, parallel = 4)
anova(betadisper(unweighted_re_412month_dist, group = metadata_re_412month$Race))
anova(betadisper(unweighted_re_412month_dist, group = metadata_re_412month$Ethnicity))
...
adonis2(weighted_re_dist ~ Study, data=metadata_re, by = "margin", permutations = 499, parallel = 4)

adonis2(weighted_re_dist ~ Sequencing, data=metadata_re, by = "margin", permutations = 499, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(weighted_re_dist ~ Sequencing, data=metadata_re, by = "margin", permutations = perm, parallel = 4)

adonis2(weighted_re_dist ~ Primer_set, data=metadata_re, by = "margin", permutations = 499, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(weighted_re_dist ~ Primer_set, data=metadata_re, by = "margin", permutations = perm, parallel = 4)

adonis2(weighted_re_dist ~ `forward sequence count`, data=metadata_re, by = "margin", permutations = 499, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(weighted_re_dist ~ `forward sequence count`, data=metadata_re, by = "margin", permutations = perm, parallel = 4)

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re, Study)
adonis2(weighted_re_dist ~ Sequencing + Primer_set + `forward sequence count`, data=metadata_re, by = "margin", permutations = perm, parallel = 4)
...
anova(betadisper(weighted_re_dist, group = metadata_re$Sequencing))
anova(betadisper(weighted_re_dist, group = metadata_re$Primer_set))
anova(betadisper(weighted_re_dist, group = metadata_re$`forward sequence count`))
...
perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re_04month, Study)
adonis2(weighted_re_04month_dist ~ Race + Ethnicity, data = metadata_re_04month, by = "margin", permutations = perm, parallel = 4)
anova(betadisper(weighted_re_04month_dist, group = metadata_re_04month$Race))
anova(betadisper(weighted_re_04month_dist, group = metadata_re_04month$Ethnicity))

perm = how(nperm = 499)
setBlocks(perm) = with(metadata_re_412month, Study)
adonis2(weighted_re_412month_dist ~ Race + Ethnicity, data = metadata_re_412month, by = "margin", permutations = perm, parallel = 4)
anova(betadisper(weighted_re_412month_dist, group = metadata_re_412month$Race))
anova(betadisper(weighted_re_412month_dist, group = metadata_re_412month$Ethnicity))
...
mds_otus_weighted_points2$Race = factor(mds_otus_weighted_points2$Race, 
...
geom_point(aes(shape = Ethnicity, 
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "<b>Race: p = 0.004, R<sup>2</sup> = 0.05%</b><br>
           Ethnicity: p = 0.720, R<sup>2</sup> = 0.01%<br>
...
mds_otus_weighted_points2_age$Host_age_cat_new = factor(mds_otus_weighted_points2_age$Host_age_cat_new, 
                                    levels = c("1-5.9 weeks", "6 weeks-2.9 months", "3-11.9 months", "12-35.9 months", "3-11.9 years"))
mds_otus_weighted_points2_age2$Host_age_cat_new2 = factor(mds_otus_weighted_points2_age2$Host_age_cat_new2, 
                                    levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months", "3-11.9 years"))
...
weight_age <- ggplot(mds_otus_weighted_points2_age, 
  ...
  stat_ellipse(aes(x = MDS1, y = MDS2, group = Host_age_cat_new, 
               ...
               type = "t", level = 0.95, show.legend = F) +
  annotate(geom = "richtext", fill = NA, label.color = NA,
...
weight_study <- ggplot(mds_otus_weighted_points2, 
  ...
  geom_point(size=3) + scale_color_manual(values = c("#1b9e77", "#d95f02",
                              "#7570b3", "#e7298a",
                              "#66a61e", "#e6ab02", 
                              "#a6761d", "#666666")) +
  theme(panel.background = element_rect(fill = 'white', 
...
weight_seq <- ggplot(mds_otus_weighted_points2, 
                      aes(x = MDS1, y = MDS2, color = Sequencing)) +
  geom_point(size=3) + scale_color_manual(values = c("#a65628", "#f781bf",
                              "#999999")) +
  theme(panel.background = element_rect(fill = 'white', 
                                        colour = 'black'), 
        legend.key=element_blank()) + 
  theme(axis.title.x=element_text(size=rel(2)), 
        ...
        legend.text = element_text(size = rel(1.5))) + 
  ggtitle("Weighted UniFrac") +
  stat_ellipse(aes(x = MDS1, y = MDS2, group = Sequencing, 
                   linetype = Sequencing), 
               type = "t", level = 0.95)

weight_prime <- ggplot(mds_otus_weighted_points2, 
                      aes(x = MDS1, y = MDS2, color = Primer_set)) +
  geom_point(size=3) + scale_color_manual(values = c("#386cb0", "#f0027f")) +
  theme(panel.background = element_rect(fill = 'white', 
                                        colour = 'black'), 
        legend.key=element_blank()) + 
  theme(axis.title.x=element_text(size=rel(2)), 
        ...
        legend.text = element_text(size = rel(1.5))) + 
  ggtitle("Weighted UniFrac") +
  stat_ellipse(aes(x = MDS1, y = MDS2, group = Primer_set, 
                   linetype = Primer_set), 
               type = "t", level = 0.95)

mds_otus_weighted_points2_depth = mds_otus_weighted_points2 %>% 
  mutate(Depth = case_when(`forward sequence count` < 20000 ~ 
                                        "Low", 
                                      `forward sequence count` < 50000 ~ 
                                        "Medium",
                                      `forward sequence count` < 691642 ~ 
                                        "High"))

mds_otus_weighted_points2_depth$Depth = factor(mds_otus_weighted_points2_depth$Depth, levels = c("Low", "Medium", "High"))

weight_depth <- ggplot(mds_otus_weighted_points2_depth, 
                      aes(x = MDS1, y = MDS2, color = Depth)) +
  geom_point(size=3) + scale_color_manual(values = c("#41b6c4", "#225ea8", "#081d58")) +
  theme(panel.background = element_rect(fill = 'white', 
                                        colour = 'black'), 
        legend.key=element_blank()) + 
  theme(axis.title.x=element_text(size=rel(2)), 
        ...
        legend.text = element_text(size = rel(1.5))) + 
  ggtitle("Weighted UniFrac") +
  stat_ellipse(aes(x = MDS1, y = MDS2, group = Depth, 
                   linetype = Depth), 
               type = "t", level = 0.95)
```
...
mds_otus_unweighted_points2_age = mds_otus_unweighted_points2 %>% 
  ...
  mutate(Host_age_cat_new = case_when(Host_age_cat == "3-6 years" ~ 
                                      ...
                                      Host_age_cat == "1-3 years" ~
                                        "1-2.9 years"))
mds_otus_unweighted_points2_age2 = mds_otus_unweighted_points2 %>% 
  filter(Host_age_cat != "NA") %>%  
  mutate(Host_age_cat_new2 = case_when(Host_age_cat == "3-6 years" ~ 
                                        "3-11.9 years", 
                                      Host_age_cat == "6-12 years" ~ 
...
mds_otus_unweighted_points2_age$Host_age_cat_new = factor(mds_otus_unweighted_points2_age$Host_age_cat_new, 
                                    levels = c("1-5.9 weeks", "6 weeks-2.9 months", "3-11.9 months", "12-35.9 months", "3-11.9 years"))
mds_otus_unweighted_points2_age2$Host_age_cat_new2 = factor(mds_otus_unweighted_points2_age2$Host_age_cat_new2, 
                                    levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months", "3-11.9 years"))
mds_otus_unweighted_points2_age$Host_age_cat2 = factor(mds_otus_unweighted_points2_age$Host_age_cat2, 
                                    levels = c("1-6 weeks", "6 weeks-3 months", "3-6 months",
                                               "6-9 months", "9-12 months",
                                                "1-3 years", "3-12 years"))
mds_otus_unweighted_points2_age$Race = factor(mds_otus_unweighted_points2_age$Race, 
                                    levels = c("White", "Black", 
                                               "Asian/Pacific Islander"))
mds_otus_unweighted_points2_age2$Race = factor(mds_otus_unweighted_points2_age2$Race, 
                                    levels = c("White", "Black", 
                                               "Asian/Pacific Islander"))
...
mds_otus_unweighted_points2_age_03month = mds_otus_unweighted_points2_age2 %>% 
  filter(Host_age_cat_new2 == "0-2.9 months")
...
mds_otus_unweighted_points2_age_012month = mds_otus_unweighted_points2_age2 %>% 
  filter(Host_age_cat_new2 == "0-2.9 months" | Host_age_cat_new2 == "3-11.9 months")
...
mds_otus_unweighted_points2_age_13year = mds_otus_unweighted_points2_age %>% 
  filter(Host_age_cat2 == "1-3 years")
...
mds_otus_unweighted_points2_sex = mds_otus_unweighted_points2 %>% 
  filter(host_sex != "NA")
...
mds_otus_unweighted_points2_del = mds_otus_unweighted_points2 %>% 
  filter(host_del_route != "NA")
...
mds_otus_unweighted_points2_feed = mds_otus_unweighted_points2 %>% 
  filter(Breastfeeding != "NA")
...
unweight_seq <- ggplot(mds_otus_unweighted_points2, 
                      aes(x = MDS1, y = MDS2, color = Sequencing)) +
  geom_point(size=3) + scale_color_manual(values = c("#a65628", "#f781bf",
                              "#999999")) +
  theme(panel.background = element_rect(fill = 'white', 
...
unweight_prime <- ggplot(mds_otus_unweighted_points2, 
                      aes(x = MDS1, y = MDS2, color = Primer_set)) +
  geom_point(size=3) + scale_color_manual(values = c("#386cb0", "#f0027f")) +
  theme(panel.background = element_rect(fill = 'white', 
                                        colour = 'black'), 
...
mds_otus_unweighted_points2_depth = mds_otus_unweighted_points2 %>% 
  mutate(Depth = case_when(`forward sequence count` < 20000 ~ 
                                        "Low", 
                                      `forward sequence count` < 50000 ~ 
                                        "Medium",
                                      `forward sequence count` < 691642 ~ 
                                        "High"))
mds_otus_unweighted_points2_depth$Depth = factor(mds_otus_unweighted_points2_depth$Depth, levels = c("Low", "Medium", "High"))

unweight_depth <- ggplot(mds_otus_unweighted_points2_depth, 
                      aes(x = MDS1, y = MDS2, color = Depth)) +
  geom_point(size=3) + scale_color_manual(values = c("#41b6c4", "#225ea8", "#081d58")) +
  theme(panel.background = element_rect(fill = 'white', 
                                        colour = 'black'), 
...
```{r nmds_seq_combo}
tiff(file="nmds_plot_seq.tif", res=150, width=18, height=9, units="in")
legend1 = get_legend(weight_seq + theme(legend.box.margin = margin(0, 0, 0, 12)))
col1 = plot_grid(weight_seq + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('A'), 
                 label_size = 20)
col2 = plot_grid(unweight_seq + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('B'), 
                 label_size = 20) 
plot = plot_grid(col1, col2, legend1, nrow = 1, ncol = 3, 
          rel_widths = c(1.5, 1.5, 0.75), align = "h", axis = "t")
...
```{r nmds_prime_combo}
tiff(file="nmds_plot_prime.tif", res=150, width=18, height=9, units="in")
legend1 = get_legend(weight_prime + theme(legend.box.margin = margin(0, 0, 0, 12)))
col1 = plot_grid(weight_prime + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('A'), 
                 label_size = 20)
col2 = plot_grid(unweight_prime + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('B'), 
                 label_size = 20) 
plot = plot_grid(col1, col2, legend1, nrow = 1, ncol = 3, 
          rel_widths = c(1.5, 1.5, 0.75), align = "h", axis = "t")
...
```{r nmds_depth_combo}
tiff(file="nmds_plot_depth.tif", res=150, width=18, height=9, units="in")
legend1 = get_legend(weight_depth + theme(legend.box.margin = margin(0, 0, 0, 12)))
col1 = plot_grid(weight_depth + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('A'), 
                 label_size = 20)
col2 = plot_grid(unweight_depth + theme(legend.position = "none"),
                 nrow = 1, ncol = 1, labels = c('B'), 
                 label_size = 20) 
plot = plot_grid(col1, col2, legend1, nrow = 1, ncol = 3, 
          rel_widths = c(1.5, 1.5, 0.75), align = "h", axis = "t")
...
write_csv(alpha_all, "alpha_diversity_values.csv")

##Dataset check
fs = lm(faith_pd ~ Study, data=alpha_all, na.action = na.omit)
Anova(fs)

os = lm(observed_features ~ Study, data=alpha_all, na.action = na.omit)
Anova(os)

ss = lm(shannon_entropy ~ Study, data=alpha_all, na.action = na.omit)
Anova(ss)

ps = lm(pielou_evenness ~ Study, data=alpha_all, na.action = na.omit)
Anova(ps)

cs = lm(chao1 ~ Study, data=alpha_all, na.action = na.omit)
Anova(cs)

fc = lmer(faith_pd ~ Sequencing + Primer_set + `forward sequence count` + (1|Study) + (1|host_subject_id), data=alpha_all, na.action = na.omit)
Anova(fc)

oc = lmer(observed_features ~ Sequencing + Primer_set + `forward sequence count` + (1|Study) + (1|host_subject_id), data=alpha_all, na.action = na.omit)
Anova(oc)

sc = lmer(shannon_entropy ~ Sequencing + Primer_set + `forward sequence count` + (1|Study) + (1|host_subject_id), data=alpha_all, na.action = na.omit)
Anova(sc)

pc = lmer(pielou_evenness ~ Sequencing + Primer_set + `forward sequence count` + (1|Study) + (1|host_subject_id), data=alpha_all, na.action = na.omit)
Anova(pc)

cc = lmer(chao1 ~ Sequencing + Primer_set + `forward sequence count` + (1|Study) + (1|host_subject_id), data=alpha_all, na.action = na.omit)
Anova(cc)
...
alpha_onem_threem = alpha_all %>% 
  filter(Host_age_cat == "0-1 week" |
           Host_age_cat == "1-6 weeks" |
           Host_age_cat == "6 weeks-3 months")
alpha_zero_fourm = alpha_all %>% 
  filter(Host_age_cat3 == "0-1 week" |
           Host_age_cat3 == "1-6 weeks" |
           Host_age_cat3 == "6 weeks-3 months" |
           Host_age_cat3 == "3-4 months")
alpha_four_twelvem = alpha_all %>% 
  filter(Host_age_cat3 == "4-6 months" |
           Host_age_cat3 == "6-9 months" |
           Host_age_cat3 == "9-12 months")
...
write_csv(f1m3mr_df, "faith_03month_race.csv")
f1m3me_df = broom::tidy(summary(glht(f1m3m,linfct=mcp(Ethnicity="Tukey"))))
write_csv(f1m3me_df, "faith_03month_ethnicity.csv")
...
write_csv(o1m3mr_df, "observed_03month_race.csv")
write_csv(o1m3me_df, "observed_03month_ethnicity.csv")
...
write_csv(s1m3mr_df, "shannon_03month_race.csv")
write_csv(s1m3me_df, "shannon_03month_ethnicity.csv")
...
write_csv(p1m3mr_df, "pielou_03month_race.csv")
write_csv(p1m3me_df, "pielou_03month_ethnicity.csv")
...
write_csv(c1m3mr_df, "chao_03month_race.csv")
write_csv(c1m3me_df, "chao_03month_ethnicity.csv")
...
write_csv(f312r_df, "faith_312month_race.csv")
write_csv(f312e_df, "faith_312month_ethnicity.csv")
...
write_csv(o312r_df, "observed_312month_race.csv")
write_csv(o312e_df, "observed_312month_ethnicity.csv")
...
write_csv(s312r_df, "shannon_312month_race.csv")
write_csv(s312e_df, "shannon_312month_ethnicity.csv")
...
write_csv(c312r_df, "chao_312month_race.csv")
write_csv(c312e_df, "chao_312month_ethnicity.csv")
...
write_csv(p312r_df, "pielou_312month_race.csv")
write_csv(p312e_df, "pielou_312month_ethnicity.csv")
...
write_csv(f13r_df, "faith_13year_race.csv")
write_csv(f13e_df, "faith_13year_ethnicity.csv")
...
write_csv(o13r_df, "observed_13year_race.csv")
write_csv(o13e_df, "observed_13year_ethnicity.csv")
...
write_csv(s13r_df, "shannon_13year_race.csv")
write_csv(s13e_df, "shannon_13year_ethnicity.csv")
...
write_csv(p13r_df, "pielou_13year_race.csv")
write_csv(p13e_df, "pielou_13year_ethnicity.csv")
...
write_csv(c13r_df, "chao_13year_race.csv")
write_csv(c13e_df, "chao_13year_ethnicity.csv")
...
#0-4 months
f04m = lmer(faith_pd ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_zero_fourm, na.action = na.omit)
summary(f04m)
Anova(f04m)
summary(glht(f04m,linfct=mcp(host_del_route="Tukey")))
summary(glht(f04m,linfct=mcp(Breastfeeding="Tukey")))
f04mr_df = broom::tidy(summary(glht(f04m,linfct=mcp(Race="Tukey"))))
f04me_df = broom::tidy(summary(glht(f04m,linfct=mcp(Ethnicity="Tukey"))))

o04m = lmer(observed_features ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_zero_fourm, na.action = na.omit)
summary(o04m)
Anova(o04m)
summary(glht(o04m,linfct=mcp(host_sex="Tukey")))
summary(glht(o04m,linfct=mcp(host_del_route="Tukey")))
o04mr_df = broom::tidy(summary(glht(o04m,linfct=mcp(Race="Tukey"))))
o04me_df = broom::tidy(summary(glht(o04m,linfct=mcp(Ethnicity="Tukey"))))

s04m = lmer(shannon_entropy ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_zero_fourm, na.action = na.omit)
summary(s04m)
Anova(s04m)
summary(glht(s04m,linfct=mcp(Breastfeeding="Tukey")))
s04mr_df = broom::tidy(summary(glht(s04m,linfct=mcp(Race="Tukey"))))
s04me_df = broom::tidy(summary(glht(s04m,linfct=mcp(Ethnicity="Tukey"))))

p04m = lmer(pielou_evenness ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_zero_fourm, na.action = na.omit)
summary(p04m)
Anova(p04m)
summary(glht(p04m,linfct=mcp(host_sex="Tukey")))
summary(glht(p04m,linfct=mcp(Breastfeeding="Tukey")))
p04mr_df = broom::tidy(summary(glht(p04m,linfct=mcp(Race="Tukey"))))
p04me_df = broom::tidy(summary(glht(p04m,linfct=mcp(Ethnicity="Tukey"))))

c04m = lmer(chao1 ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_zero_fourm, na.action = na.omit)
summary(c04m)
Anova(c04m)
summary(glht(c04m,linfct=mcp(host_sex="Tukey")))
summary(glht(c04m,linfct=mcp(host_del_route="Tukey")))
c04mr_df = broom::tidy(summary(glht(c04m,linfct=mcp(Race="Tukey"))))
c04me_df = broom::tidy(summary(glht(c04m,linfct=mcp(Ethnicity="Tukey"))))

#4-12 months
f412m = lmer(faith_pd ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_four_twelvem, na.action = na.omit)
summary(f412m)
Anova(f412m)
summary(glht(f412m,linfct=mcp(Race="Tukey")))
summary(glht(f412m,linfct=mcp(host_del_route="Tukey")))
summary(glht(f412m,linfct=mcp(Breastfeeding="Tukey")))
f412mr_df = broom::tidy(summary(glht(f412m,linfct=mcp(Race="Tukey"))))
f412me_df = broom::tidy(summary(glht(f412m,linfct=mcp(Ethnicity="Tukey"))))

o412m = lmer(observed_features ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_four_twelvem, na.action = na.omit)
summary(o412m)
Anova(o412m)
summary(glht(o412m,linfct=mcp(Race="Tukey")))
summary(glht(o412m,linfct=mcp(host_del_route="Tukey")))
summary(glht(o412m,linfct=mcp(Breastfeeding="Tukey")))
o412mr_df = broom::tidy(summary(glht(o412m,linfct=mcp(Race="Tukey"))))
o412me_df = broom::tidy(summary(glht(o412m,linfct=mcp(Ethnicity="Tukey"))))

s412m = lmer(shannon_entropy ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_four_twelvem, na.action = na.omit)
summary(s412m)
Anova(s412m)
summary(glht(s412m,linfct=mcp(Race="Tukey")))
summary(glht(s412m,linfct=mcp(host_del_route="Tukey")))
summary(glht(s412m,linfct=mcp(Breastfeeding="Tukey")))
s412mr_df = broom::tidy(summary(glht(s412m,linfct=mcp(Race="Tukey"))))
s412me_df = broom::tidy(summary(glht(s412m,linfct=mcp(Ethnicity="Tukey"))))

p412m = lmer(pielou_evenness ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_four_twelvem, na.action = na.omit)
summary(p412m)
Anova(p412m)
summary(glht(p412m,linfct=mcp(Race="Tukey")))
summary(glht(p412m,linfct=mcp(Breastfeeding="Tukey")))
p412mr_df = broom::tidy(summary(glht(p412m,linfct=mcp(Race="Tukey"))))
p412me_df = broom::tidy(summary(glht(p412m,linfct=mcp(Ethnicity="Tukey"))))

c412m = lmer(chao1 ~ Race + Ethnicity + host_sex + host_del_route + Breastfeeding + (1|Study), data=alpha_four_twelvem, na.action = na.omit)
summary(c412m)
Anova(c412m)
summary(glht(c412m,linfct=mcp(Race="Tukey")))
summary(glht(c412m,linfct=mcp(Breastfeeding="Tukey")))
summary(glht(c412m,linfct=mcp(host_del_route="Tukey")))
c412mr_df = broom::tidy(summary(glht(c412m,linfct=mcp(Race="Tukey"))))
c412me_df = broom::tidy(summary(glht(c412m,linfct=mcp(Ethnicity="Tukey"))))
```
...
alpha_all_age$Host_age_cat_new = factor(alpha_all_age$Host_age_cat_new, 
                                    levels = c("0-0.9 week", "1-5.9 weeks", "6 weeks-2.9 months", "3-11.9 months", "12-35.9 months", "3-11.9 years"))
...
alpha_all_age_13year = alpha_all_age %>% 
  filter(Host_age_cat_new == "12-35.9 months")
...
tiff(file="alpha_effect_race_eth_0months3months.tif", 
     res=300, width=15, height=8, units="in")
...
faithr_df$model = factor(faithr_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
shannonr_df = rbind(s1m3mr_df3, s312r_df3, s13r_df3)
shannonr_df$model = factor(shannonr_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
pielour_df = rbind(p1m3mr_df3, p312r_df3, p13r_df3)
pielour_df$model = factor(pielour_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
observedr_df = rbind(o1m3mr_df3, o312r_df3, o13r_df3)
observedr_df$model = factor(observedr_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
chaor_df = rbind(c1m3mr_df3, c312r_df3, c13r_df3)
chaor_df$model = factor(chaor_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
...
faithr_dw = dwplot(faithr_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  scale_color_manual(name = "Metric", 
                                        ...
                                        labels = c("Faith's PD")) +
  scale_y_discrete(labels = c("White\n- Black", 
                              "White\n- Asian/PI", 
                              "Black\n- Asian/PI")) +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "0-2.9 months: p = 0.718<br>
           3-11.9 months: p = 0.233<br>
           <span style='color: blue1;'><b><i>12-35.9 months: p = 0.006</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
observedr_dw = dwplot(observedr_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           ...
           12-35.9 months: p = 0.001</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
chaor_dw = dwplot(chaor_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           ...
           12-35.9 months: p = 0.002</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
shannonr_dw = dwplot(shannonr_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           ...
           12-35.9 months: p = 0.020</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
faithe_df$model = factor(faithe_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
shannone_df = rbind(s1m3me_df3, s312e_df3, s13e_df3)
shannone_df$model = factor(shannone_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
pieloue_df = rbind(p1m3me_df3, p312e_df3, p13e_df3)
pieloue_df$model = factor(pieloue_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
observede_df = rbind(o1m3me_df3, o312e_df3, o13e_df3)
observede_df$model = factor(observede_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
chaoe_df = rbind(c1m3me_df3, c312e_df3, c13e_df3) 
chaoe_df$model = factor(chaoe_df$model, levels = c("0-2.9 months", "3-11.9 months", "12-35.9 months")) 
...
faithe_dw = dwplot(faithe_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           ...
           <span style='color: blue1;'><b><i>12-35.9 months: p = 0.015</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
observede_dw = dwplot(observede_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           ...
           <span style='color: blue1;'><b><i>12-35.9 months: p = 0.023</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
chaoe_dw = dwplot(chaoe_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           ...
           <span style='color: blue1;'><b><i>12-35.9 months: p = 0.018</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
shannone_dw = dwplot(shannone_df, vline = geom_vline(xintercept = 0, color = "gray40",
  ...
  annotate(geom = "richtext", fill = NA, label.color = NA,
           ...
           <span style='color: blue1;'><b><i>12-35.9 months: p = 0.042</b></i></span>", 
           x = Inf, y = -Inf, size = 6,
           hjust = 1, vjust = 0)
...
#####Phyla 0-1 weeks
```{r phyla_abund_01week}
phyla = read_tsv("phyla.txt")
phyla_ra = phyla %>% column_to_rownames("SampleID") %>% 
  decostand(method = "total", MARGIN = 1) %>% rownames_to_column()
phyla_meta = metadata_re %>% 
  inner_join(phyla_ra, by = c("SampleID" = "rowname"))

phyla_meta_01week = phyla_meta %>% 
  filter(Host_age_cat == "0-1 week") %>% 
  as.data.frame()

phyla_meta_01week$Race = as.factor(phyla_meta_01week$Race)

phyla_meta_01week$Ethnicity = as.factor(phyla_meta_01week$Ethnicity)

race_estimatematrix = mat.or.vec(66,2)
race_pvaluematrix = mat.or.vec(66,2)
ethnicity_estimatematrix = mat.or.vec(66,2)
ethnicity_pvaluematrix = mat.or.vec(66,2)

for(i in 85:111) {
  variable = phyla_meta_01week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = phyla_meta_01week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(phyla_meta_01week)[i]
  race_pvaluematrix[i-45,1] = names(phyla_meta_01week)[i]
  ethnicity_estimatematrix[i-45,1] = names(phyla_meta_01week)[i]
  ethnicity_pvaluematrix[i-45,1] = names(phyla_meta_01week)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_phyla_01week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_phyla_01week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_phyla_01week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_phyla_01week.csv")

```

#####Phyla 1-6 weeks
```{r phyla_abund_16week}
phyla_meta = metadata_re %>% 
  inner_join(phyla_ra, by = c("SampleID" = "rowname"))
phyla_meta_16week = phyla_meta %>% 
  filter(Host_age_cat == "1-6 weeks") %>% 
  as.data.frame()

phyla_meta_16week$Race = as.factor(phyla_meta_16week$Race)

phyla_meta_16week$Ethnicity = as.factor(phyla_meta_16week$Ethnicity)
...
for(i in 46:111) {
  variable = phyla_meta_16week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = phyla_meta_16week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
...
race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_phyla_16week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_phyla_16week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_phyla_16week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_phyla_16week.csv")
```

#####Phyla 6 weeks-3 months
```{r phyla_abund_6week3month}
phyla_meta = metadata_re %>% 
  inner_join(phyla_ra, by = c("SampleID" = "rowname"))
phyla_meta_6week3month = phyla_meta %>% 
  filter(Host_age_cat == "6 weeks-3 months") %>% 
  as.data.frame()

phyla_meta_6week3month$Race = as.factor(phyla_meta_6week3month$Race)

phyla_meta_6week3month$Ethnicity = as.factor(phyla_meta_6week3month$Ethnicity)
...
for(i in 46:111) {
  variable = phyla_meta_6week3month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = phyla_meta_6week3month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(phyla_meta_6week3month)[i]
  race_pvaluematrix[i-45,1] = names(phyla_meta_6week3month)[i]
  ethnicity_estimatematrix[i-45,1] = names(phyla_meta_6week3month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(phyla_meta_6week3month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_phyla_6week3month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_phyla_6week3month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_phyla_6week3month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_phyla_6week3month.csv")
```

#####Phyla 3-12 months
```{r phyla_abund_312month}
phyla_meta = metadata_re %>% 
  inner_join(phyla_ra, by = c("SampleID" = "rowname"))
phyla_meta_312month = phyla_meta %>% 
  filter(Host_age_cat == "3-12 months") %>% 
  as.data.frame()

phyla_meta_312month$Race = as.factor(phyla_meta_312month$Race)

phyla_meta_312month$Ethnicity = as.factor(phyla_meta_312month$Ethnicity)
...
for(i in 46:111) {
  variable = phyla_meta_312month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = phyla_meta_312month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(phyla_meta_312month)[i]
  race_pvaluematrix[i-45,1] = names(phyla_meta_312month)[i]
  ethnicity_estimatematrix[i-45,1] = names(phyla_meta_312month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(phyla_meta_312month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_phyla_312month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_phyla_312month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_phyla_312month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_phyla_312month.csv")

```

#####Phyla 1-3 years
```{r phyla_abund_13year}
phyla_meta = metadata_re %>% 
  inner_join(phyla_ra, by = c("SampleID" = "rowname"))
phyla_meta_13year = phyla_meta %>% 
  filter(Host_age_cat == "1-3 years") %>% 
  as.data.frame()

phyla_meta_13year$Race = as.factor(phyla_meta_13year$Race)

phyla_meta_13year$Ethnicity = as.factor(phyla_meta_13year$Ethnicity)
...
for(i in 46:111) {
  variable = phyla_meta_13year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = phyla_meta_13year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(phyla_meta_13year)[i]
  race_pvaluematrix[i-45,1] = names(phyla_meta_13year)[i]
  ethnicity_estimatematrix[i-45,1] = names(phyla_meta_13year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(phyla_meta_13year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_phyla_13year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_phyla_13year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_phyla_13year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_phyla_13year.csv")
```

#####Phyla 3-6 years
```{r phyla_abund_36year}
phyla_meta = metadata_re %>% 
  inner_join(phyla_ra, by = c("SampleID" = "rowname"))
phyla_meta_36year = phyla_meta %>% 
  filter(Host_age_cat == "3-6 years") %>% 
  as.data.frame()

phyla_meta_36year$Race = as.factor(phyla_meta_36year$Race)

phyla_meta_36year$Ethnicity = as.factor(phyla_meta_36year$Ethnicity)
...
for(i in 46:111) {
  variable = phyla_meta_36year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = phyla_meta_36year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(phyla_meta_36year)[i]
  race_pvaluematrix[i-45,1] = names(phyla_meta_36year)[i]
  ethnicity_estimatematrix[i-45,1] = names(phyla_meta_36year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(phyla_meta_36year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_phyla_36year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_phyla_36year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_phyla_36year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_phyla_36year.csv")

```

#####Phyla 6-12 years
```{r phyla_abund_612year}
phyla_meta = metadata_re %>% 
  inner_join(phyla_ra, by = c("SampleID" = "rowname"))
phyla_meta_612year = phyla_meta %>% 
  filter(Host_age_cat == "6-12 years") %>% 
  as.data.frame()

phyla_meta_612year$Race = as.factor(phyla_meta_612year$Race)

phyla_meta_612year$Ethnicity = as.factor(phyla_meta_612year$Ethnicity)
...
for(i in 46:111) {
  variable = phyla_meta_612year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = phyla_meta_612year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(phyla_meta_612year)[i]
  race_pvaluematrix[i-45,1] = names(phyla_meta_612year)[i]
  ethnicity_estimatematrix[i-45,1] = names(phyla_meta_612year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(phyla_meta_612year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_phyla_612year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_phyla_612year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_phyla_612year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_phyla_612year.csv")
```
...
#####Family 0-1 weeks
```{r family_abund_01week}
family = read_tsv("family.txt")
family_ra = family %>% column_to_rownames("SampleID") %>% 
  decostand(method = "total", MARGIN = 1) %>% rownames_to_column()
family_meta = metadata_re %>% 
  inner_join(family_ra, by = c("SampleID" = "rowname"))

family_meta_01week = family_meta %>% 
  filter(Host_age_cat == "0-1 week") %>% 
  as.data.frame()

family_meta_01week$Race = as.factor(family_meta_01week$Race)

family_meta_01week$Ethnicity = as.factor(family_meta_01week$Ethnicity)

race_estimatematrix = mat.or.vec(605,2)
race_pvaluematrix = mat.or.vec(605,2)
ethnicity_estimatematrix = mat.or.vec(605,2)
ethnicity_pvaluematrix = mat.or.vec(605,2)

for(i in 554:650) {
  variable = family_meta_01week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = family_meta_01week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(family_meta_01week)[i]
  race_pvaluematrix[i-45,1] = names(family_meta_01week)[i]
  ethnicity_estimatematrix[i-45,1] = names(family_meta_01week)[i]
  ethnicity_pvaluematrix[i-45,1] = names(family_meta_01week)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_family_01week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3],
statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_family_01week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_family_01week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_family_01week.csv")

a = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_01week, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Ethnicity="Tukey")))

b = glmmTMB(`k__Bacteria;p__Actinobacteria;c__Actinobacteria;o__Bifidobacteriales;f__Bifidobacteriaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_01week, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Ethnicity="Tukey")))

c = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Lactobacillaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_01week, family = nbinom2)
summary(c)
Anova(c)
summary(glht(c,linfct=mcp(Ethnicity="Tukey")))

d = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__[Paraprevotellaceae]` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_01week, family = nbinom2)
summary(d)
Anova(d)
summary(glht(d,linfct=mcp(Ethnicity="Tukey")))
```

#####Family 1-6 weeks
```{r family_abund_16week}
family_meta_16week = family_meta %>% 
  filter(Host_age_cat == "1-6 weeks") %>% 
  as.data.frame()

family_meta_16week$Race = as.factor(family_meta_16week$Race)

family_meta_16week$Ethnicity = as.factor(family_meta_16week$Ethnicity)
...
for(i in 588:650) {
  variable = family_meta_16week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = family_meta_16week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(family_meta_16week)[i]
  race_pvaluematrix[i-45,1] = names(family_meta_16week)[i]
  ethnicity_estimatematrix[i-45,1] = names(family_meta_16week)[i]
  ethnicity_pvaluematrix[i-45,1] = names(family_meta_16week)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_family_16week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_family_16week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_family_16week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_family_16week.csv")

a = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;__` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))

b = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Porphyromonadaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Race="Tukey")))

c = glmmTMB(`k__Bacteria;p__Verrucomicrobia;c__Verrucomicrobiae;o__Verrucomicrobiales;f__Verrucomicrobiaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(c)
Anova(c)
summary(glht(c,linfct=mcp(Race="Tukey")))

d = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Christensenellaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(d)
Anova(d)
summary(glht(d,linfct=mcp(Race="Tukey")))

e = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__[Barnesiellaceae]` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(e)
Anova(e)
summary(glht(e,linfct=mcp(Race="Tukey")))

f = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Pasteurellales;f__Pasteurellaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(f)
Anova(f)
summary(glht(f,linfct=mcp(Race="Tukey")))

g = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Bacillales;f__Bacillaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(g)
Anova(g)
summary(glht(g,linfct=mcp(Race="Tukey")))

h = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(h)
Anova(h)
summary(glht(h,linfct=mcp(Ethnicity="Tukey")))

i = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Streptococcaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(i)
Anova(i)
summary(glht(i,linfct=mcp(Ethnicity="Tukey")))

j = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Pseudomonadales;f__Moraxellaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(j)
Anova(j)
summary(glht(j,linfct=mcp(Ethnicity="Tukey")))

k = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Lactobacillaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(k)
Anova(k)
summary(glht(k,linfct=mcp(Ethnicity="Tukey")))

l = glmmTMB(`k__Bacteria;p__Actinobacteria;c__Actinobacteria;o__Actinomycetales;__` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(l)
Anova(l)
summary(glht(l,linfct=mcp(Ethnicity="Tukey")))

m = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Enterococcaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_16week, family = nbinom2)
summary(m)
Anova(m)
summary(glht(m,linfct=mcp(Ethnicity="Tukey")))



```

#####Family 6 weeks-3 months
```{r family_abund_6week3month}
family_meta_6week3month = family_meta %>% 
  filter(Host_age_cat == "6 weeks-3 months") %>% 
  as.data.frame()

family_meta_6week3month$Race = as.factor(family_meta_6week3month$Race)

family_meta_6week3month$Ethnicity = as.factor(family_meta_6week3month$Ethnicity)
...
for(i in 270:650) {
  variable = family_meta_6week3month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = family_meta_6week3month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(family_meta_6week3month)[i]
  race_pvaluematrix[i-45,1] = names(family_meta_6week3month)[i]
  ethnicity_estimatematrix[i-45,1] = names(family_meta_6week3month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(family_meta_6week3month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_family_6week3month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_family_6week3month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_family_6week3month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_family_6week3month.csv")
```

#####Family 3-12 months
```{r family_abund_312month}
family_meta_312month = family_meta %>% 
  filter(Host_age_cat == "3-12 months") %>% 
  as.data.frame()

family_meta_312month$Race = as.factor(family_meta_312month$Race)

family_meta_312month$Ethnicity = as.factor(family_meta_312month$Ethnicity)
...
for(i in 540:650) {
  variable = family_meta_312month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = family_meta_312month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(family_meta_312month)[i]
  race_pvaluematrix[i-45,1] = names(family_meta_312month)[i]
  ethnicity_estimatematrix[i-45,1] = names(family_meta_312month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(family_meta_312month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_family_312month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_family_312month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_family_312month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_family_312month.csv")
```

#####Family 1-3 years
```{r family_abund_13years}
family_meta_13year = family_meta %>% 
  filter(Host_age_cat == "1-3 years") %>% 
  as.data.frame()

family_meta_13year$Race = as.factor(family_meta_13year$Race)

family_meta_13year$Ethnicity = as.factor(family_meta_13year$Ethnicity)
...
for(i in 634:650) {
  variable = family_meta_13year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = family_meta_13year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(family_meta_13year)[i]
  race_pvaluematrix[i-45,1] = names(family_meta_13year)[i]
  ethnicity_estimatematrix[i-45,1] = names(family_meta_13year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(family_meta_13year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_family_13year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_family_13year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_family_13year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_family_13year.csv")
```

#####Family 3-6 years
```{r family_abund_36year}
family_meta_36year = family_meta %>% 
  filter(Host_age_cat == "3-6 years") %>% 
  as.data.frame()

family_meta_36year$Race = as.factor(family_meta_36year$Race)

family_meta_36year$Ethnicity = as.factor(family_meta_36year$Ethnicity)
...
for(i in 46:650) {
  variable = family_meta_36year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = family_meta_36year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(family_meta_36year)[i]
  race_pvaluematrix[i-45,1] = names(family_meta_36year)[i]
  ethnicity_estimatematrix[i-45,1] = names(family_meta_36year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(family_meta_36year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_family_36year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_family_36year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_family_36year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_family_36year.csv")
```

#####Family 6-12 years
```{r family_abund_612year}
family_meta_612year = family_meta %>% 
  filter(Host_age_cat == "6-12 years") %>% 
  as.data.frame()

family_meta_612year$Race = as.factor(family_meta_612year$Race)

family_meta_612year$Ethnicity = as.factor(family_meta_612year$Ethnicity)
...
for(i in 217:650) {
  variable = family_meta_612year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = family_meta_612year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(family_meta_612year)[i]
  race_pvaluematrix[i-45,1] = names(family_meta_612year)[i]
  ethnicity_estimatematrix[i-45,1] = names(family_meta_612year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(family_meta_612year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_family_612year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_family_612year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_family_612year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_family_612year.csv")

a = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__[Tissierellaceae]` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_612year, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))

b = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_612year, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Race="Tukey")))

c = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Alphaproteobacteria;o__RF32;f__` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_612year, family = nbinom2)
summary(c)
Anova(c)
summary(glht(c,linfct=mcp(Race="Tukey")))

d = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Porphyromonadaceae` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_612year, family = nbinom2)
summary(d)
Anova(d)
summary(glht(d,linfct=mcp(Ethnicity="Tukey")))

e = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;__` ~ Race + Ethnicity + (1|Study), 
            data = family_meta_612year, family = nbinom2)
summary(e)
Anova(e)
summary(glht(e,linfct=mcp(Ethnicity="Tukey")))
```
...
#####Genera 0-1 weeks
```{r genera_abund_01week}
genus = read_tsv("genus.txt")
genus_ra = genus %>% column_to_rownames("SampleID") %>% 
  decostand(method = "total", MARGIN = 1) %>% rownames_to_column()
genus_meta = metadata_re %>% 
  inner_join(genus_ra, by = c("SampleID" = "rowname"))

genus_meta_01week = genus_meta %>% 
  filter(Host_age_cat == "0-1 week") %>% 
  as.data.frame()

genus_meta_01week$Race = as.factor(genus_meta_01week$Race)

genus_meta_01week$Ethnicity = as.factor(genus_meta_01week$Ethnicity)

race_estimatematrix = mat.or.vec(1158,2)
race_pvaluematrix = mat.or.vec(1158,2)
ethnicity_estimatematrix = mat.or.vec(1158,2)
ethnicity_pvaluematrix = mat.or.vec(1158,2)

for(i in 1130:1203) {
  variable = genus_meta_01week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = genus_meta_01week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(genus_meta_01week)[i]
  race_pvaluematrix[i-45,1] = names(genus_meta_01week)[i]
  ethnicity_estimatematrix[i-45,1] = names(genus_meta_01week)[i]
  ethnicity_pvaluematrix[i-45,1] = names(genus_meta_01week)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_genus_01week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_genus_01week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_genus_01week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_genus_01week.csv")

genus_meta_01week_B = genus_meta_01week %>% filter(Race == "Black")

Prev_B = summary(genus_meta_01week_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_B_std = sd(genus_meta_01week_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_B_prev = sum(genus_meta_01week_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella'>0)

Veil_B = summary(genus_meta_01week_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_B_std = sd(genus_meta_01week_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_B_prev = sum(genus_meta_01week_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella'>0)

genus_meta_01week_W = genus_meta_01week %>% filter(Race == "White")

Prev_W = summary(genus_meta_01week_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_W_std = sd(genus_meta_01week_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_W_prev = sum(genus_meta_01week_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella'>0)

Veil_W = summary(genus_meta_01week_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_W_std = sd(genus_meta_01week_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_W_prev = sum(genus_meta_01week_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella'>0)
...
#####Genera 1-6 weeks
```{r genera_abund_16week}
genus_meta_16week = genus_meta %>% 
  filter(Host_age_cat == "1-6 weeks") %>% 
  as.data.frame()

genus_meta_16week$Race = as.factor(genus_meta_16week$Race)

genus_meta_16week$Ethnicity = as.factor(genus_meta_16week$Ethnicity)
...
for(i in 1050:1203) {
  variable = genus_meta_16week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = genus_meta_16week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(genus_meta_16week)[i]
  race_pvaluematrix[i-45,1] = names(genus_meta_16week)[i]
  ethnicity_estimatematrix[i-45,1] = names(genus_meta_16week)[i]
  ethnicity_pvaluematrix[i-45,1] = names(genus_meta_16week)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_genus_16week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_genus_16week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_genus_16week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_genus_16week.csv")

genus_meta_16week_B = genus_meta_16week %>% filter(Race == "Black")

Prev_B = summary(genus_meta_16week_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_B_std = sd(genus_meta_16week_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_B_prev = sum(genus_meta_16week_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella'>0)

Veil_B = summary(genus_meta_16week_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_B_std = sd(genus_meta_16week_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_B_prev = sum(genus_meta_16week_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella'>0)

genus_meta_16week_W = genus_meta_16week %>% filter(Race == "White")

Prev_W = summary(genus_meta_16week_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_W_std = sd(genus_meta_16week_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_W_prev = sum(genus_meta_16week_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella'>0)

Veil_W = summary(genus_meta_16week_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_W_std = sd(genus_meta_16week_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_W_prev = sum(genus_meta_16week_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella'>0)
```

#####Genera 6 weeks-3 months
```{r genera_abund_6week3month}
genus_meta_6week3month = genus_meta %>% 
  filter(Host_age_cat == "6 weeks-3 months") %>% 
  as.data.frame()

genus_meta_6week3month$Race = as.factor(genus_meta_6week3month$Race)

genus_meta_6week3month$Ethnicity = as.factor(genus_meta_6week3month$Ethnicity)
...
for(i in 852:1203) {
  variable = genus_meta_6week3month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = genus_meta_6week3month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(genus_meta_6week3month)[i]
  race_pvaluematrix[i-45,1] = names(genus_meta_6week3month)[i]
  ethnicity_estimatematrix[i-45,1] = names(genus_meta_6week3month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(genus_meta_6week3month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_genus_6week3month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_genus_6week3month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_genus_6week3month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_genus_6week3month.csv")

genus_meta_6week3month_B = genus_meta_6week3month %>% filter(Race == "Black")

Prev_B = summary(genus_meta_6week3month_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_B_std = sd(genus_meta_6week3month_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_B_prev = sum(genus_meta_6week3month_B$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella'>0)

Veil_B = summary(genus_meta_6week3month_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_B_std = sd(genus_meta_6week3month_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_B_prev = sum(genus_meta_6week3month_B$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella'>0)

genus_meta_6week3month_W = genus_meta_6week3month %>% filter(Race == "White")

Prev_W = summary(genus_meta_6week3month_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_W_std = sd(genus_meta_6week3month_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella')
Prev_W_prev = sum(genus_meta_6week3month_W$'k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella'>0)

Veil_W = summary(genus_meta_6week3month_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_W_std = sd(genus_meta_6week3month_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella')
Veil_W_prev = sum(genus_meta_6week3month_W$'k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella'>0)

```

#####Genera 3-12 months
```{r genera_abund_312month}
genus_meta_312month = genus_meta %>% 
  filter(Host_age_cat == "3-12 months") %>% 
  as.data.frame()

genus_meta_312month$Race = as.factor(genus_meta_312month$Race)

genus_meta_312month$Ethnicity = as.factor(genus_meta_312month$Ethnicity)
...
for(i in 1171:1203) {
  variable = genus_meta_312month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = genus_meta_312month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(genus_meta_312month)[i]
  race_pvaluematrix[i-45,1] = names(genus_meta_312month)[i]
  ethnicity_estimatematrix[i-45,1] = names(genus_meta_312month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(genus_meta_312month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_genus_312month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_genus_312month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_genus_312month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_genus_312month.csv")
```

#####Genera 1-3 years
```{r genera_abund_13years}
genus_meta_13year = genus_meta %>% 
  filter(Host_age_cat == "1-3 years") %>% 
  as.data.frame()

genus_meta_13year$Race = as.factor(genus_meta_13year$Race)

genus_meta_13year$Ethnicity = as.factor(genus_meta_13year$Ethnicity)
...
for(i in 1164:1203) {
  variable = genus_meta_13year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = genus_meta_13year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(genus_meta_13year)[i]
  race_pvaluematrix[i-45,1] = names(genus_meta_13year)[i]
  ethnicity_estimatematrix[i-45,1] = names(genus_meta_13year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(genus_meta_13year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_genus_13year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_genus_13year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_genus_13year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_genus_13year.csv")

a = glmmTMB(`k__Bacteria;p__OD1;c__;o__;f__;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))

b = glmmTMB(`k__Bacteria;__;__;__;__;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Race="Tukey")))

c = glmmTMB(`k__Bacteria;p__Firmicutes;c__Erysipelotrichi;o__Erysipelotrichales;f__Erysipelotrichaceae;g__[Eubacterium]` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(c)
Anova(c)
summary(glht(c,linfct=mcp(Race="Tukey")))
...
d = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Enterobacteriales;f__Enterobacteriaceae;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(d)
Anova(d)
summary(glht(d,linfct=mcp(Race="Tukey")))

e = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__[Mogibacteriaceae];g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(e)
Anova(e)
summary(glht(e,linfct=mcp(Race="Tukey")))

f = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Prevotellaceae;g__Prevotella` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(f)
Anova(f)
summary(glht(f,linfct=mcp(Race="Tukey")))
...
g = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Streptococcaceae;g__Streptococcus` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(g)
Anova(g)
summary(glht(g,linfct=mcp(Race="Tukey")))
summary(glht(g,linfct=mcp(Ethnicity="Tukey")))

h = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Alphaproteobacteria;o__Rhodobacterales;f__Rhodobacteraceae;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(h)
Anova(h)
summary(glht(h,linfct=mcp(Race="Tukey")))

i = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Ruminococcaceae;g__Ruminococcus` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(i)
Anova(i)
summary(glht(i,linfct=mcp(Race="Tukey")))

j = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__[Odoribacteraceae];g__Butyricimonas` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(j)
Anova(j)
summary(glht(j,linfct=mcp(Race="Tukey")))
summary(glht(j,linfct=mcp(Ethnicity="Tukey")))

k = glmmTMB(`k__Bacteria;p__Firmicutes;c__Erysipelotrichi;o__Erysipelotrichales;f__Erysipelotrichaceae;g__Clostridium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(k)
Anova(k)
summary(glht(k,linfct=mcp(Race="Tukey")))
summary(glht(k,linfct=mcp(Ethnicity="Tukey")))

j = glmmTMB(`k__Bacteria;p__Actinobacteria;c__Actinobacteria;o__Bifidobacteriales;f__Bifidobacteriaceae;g__Bifidobacterium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(j)
Anova(j)
summary(glht(j,linfct=mcp(Race="Tukey")))
...
k = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Dialister` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(k)
Anova(k)
summary(glht(k,linfct=mcp(Race="Tukey")))
...
l = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Porphyromonadaceae;g__Parabacteroides` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(l)
Anova(l)
summary(glht(l,linfct=mcp(Race="Tukey")))

m = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__[Saprospirae];o__[Saprospirales];f__Saprospiraceae;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(m)
Anova(m)
summary(glht(m,linfct=mcp(Race="Tukey")))
summary(glht(m,linfct=mcp(Ethnicity="Tukey")))

n = glmmTMB(`k__Bacteria;p__Actinobacteria;c__Actinobacteria;o__Actinomycetales;f__Corynebacteriaceae;g__Corynebacterium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(n)
Anova(n)
summary(glht(n,linfct=mcp(Race="Tukey")))
summary(glht(n,linfct=mcp(Ethnicity="Tukey")))

o = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Lachnospiraceae;g__[Ruminococcus]` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(o)
Anova(o)
summary(glht(o,linfct=mcp(Race="Tukey")))

p = glmmTMB(`k__Bacteria;p__Firmicutes;c__Erysipelotrichi;o__Erysipelotrichales;f__Erysipelotrichaceae;g__Bulleidia` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(p)
Anova(p)
summary(glht(p,linfct=mcp(Race="Tukey")))

q = glmmTMB(`k__Bacteria;p__OD1;__;__;__;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(q)
Anova(q)
summary(glht(q,linfct=mcp(Race="Tukey")))

r = glmmTMB(`Unassigned;__;__;__;__;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(r)
Anova(r)
summary(glht(r,linfct=mcp(Race="Tukey")))

s = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Pasteurellales;f__Pasteurellaceae;g__Haemophilus` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(s)
Anova(s)
summary(glht(s,linfct=mcp(Race="Tukey")))

t = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Eubacteriaceae;g__Pseudoramibacter_Eubacterium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(t)
Anova(t)
summary(glht(t,linfct=mcp(Race="Tukey")))
summary(glht(t,linfct=mcp(Ethnicity="Tukey")))

u = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Peptostreptococcaceae;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(u)
Anova(u)
summary(glht(u,linfct=mcp(Race="Tukey")))
summary(glht(u,linfct=mcp(Ethnicity="Tukey")))

v = glmmTMB(`k__Bacteria;p__Tenericutes;c__Mollicutes;o__RF39;f__;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(v)
Anova(v)
summary(glht(v,linfct=mcp(Race="Tukey")))

w = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Lachnospiraceae;g__Lachnospira` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(w)
Anova(w)
summary(glht(w,linfct=mcp(Race="Tukey")))

x = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__[Odoribacteraceae];g__Odoribacter` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(x)
Anova(x)
summary(glht(x,linfct=mcp(Race="Tukey")))

y = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Deltaproteobacteria;o__Desulfovibrionales;f__Desulfovibrionaceae;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(y)
Anova(y)
summary(glht(y,linfct=mcp(Race="Tukey")))

z = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Deltaproteobacteria;o__Desulfovibrionales;f__Desulfovibrionaceae;g__Desulfovibrio` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(z)
Anova(z)
summary(glht(z,linfct=mcp(Race="Tukey")))
summary(glht(z,linfct=mcp(Ethnicity="Tukey")))

aa = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__[Paraprevotellaceae];g__[Prevotella]` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(aa)
Anova(aa)
summary(glht(aa,linfct=mcp(Race="Tukey")))

bb = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Betaproteobacteria;o__Burkholderiales;__;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(bb)
Anova(bb)
summary(glht(bb,linfct=mcp(Race="Tukey")))

cc = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Peptostreptococcaceae;g__[Clostridium]` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(cc)
Anova(cc)
summary(glht(cc,linfct=mcp(Race="Tukey")))

dd = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Ruminococcaceae;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(dd)
Anova(dd)
summary(glht(dd,linfct=mcp(Ethnicity="Tukey")))

ee = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Clostridiaceae;g__Clostridium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(ee)
Anova(ee)
summary(glht(ee,linfct=mcp(Ethnicity="Tukey")))

ff = glmmTMB(`k__Bacteria;p__Firmicutes;c__Erysipelotrichi;o__Erysipelotrichales;f__Erysipelotrichaceae;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(ff)
Anova(ff)
summary(glht(ff,linfct=mcp(Ethnicity="Tukey")))

gg = glmmTMB(`k__Bacteria;p__Proteobacteria;__;__;__;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(gg)
Anova(gg)
summary(glht(gg,linfct=mcp(Ethnicity="Tukey")))

hh = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(hh)
Anova(hh)
summary(glht(hh,linfct=mcp(Ethnicity="Tukey")))

ii = glmmTMB(`k__Bacteria;p__Verrucomicrobia;c__Verrucomicrobiae;o__Verrucomicrobiales;f__Verrucomicrobiaceae;g__Akkermansia` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(ii)
Anova(ii)
summary(glht(ii,linfct=mcp(Ethnicity="Tukey")))

jj = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Christensenellaceae;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(jj)
Anova(jj)
summary(glht(jj,linfct=mcp(Ethnicity="Tukey")))

kk = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Lachnospiraceae;g__Clostridium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(kk)
Anova(kk)
summary(glht(kk,linfct=mcp(Ethnicity="Tukey")))

ll = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Megasphaera` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(ll)
Anova(ll)
summary(glht(ll,linfct=mcp(Ethnicity="Tukey")))

mm = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Lactobacillaceae;g__Lactobacillus` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(mm)
Anova(mm)
summary(glht(mm,linfct=mcp(Ethnicity="Tukey")))

nn = glmmTMB(`k__Bacteria;p__Firmicutes;c__Erysipelotrichi;o__Erysipelotrichales;f__Erysipelotrichaceae;g__Catenibacterium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(nn)
Anova(nn)
summary(glht(nn,linfct=mcp(Ethnicity="Tukey")))

oo = glmmTMB(`Unassigned;__;__;__;__;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(oo)
Anova(oo)
summary(glht(oo,linfct=mcp(Ethnicity="Tukey")))

pp = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Rikenellaceae;g__Alistipes` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(pp)
Anova(pp)
summary(glht(pp,linfct=mcp(Ethnicity="Tukey")))

qq = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__[Tissierellaceae];g__ph2` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(qq)
Anova(qq)
summary(glht(qq,linfct=mcp(Ethnicity="Tukey")))

rr = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Phascolarctobacterium` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(rr)
Anova(rr)
summary(glht(rr,linfct=mcp(Ethnicity="Tukey")))

ss = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Streptococcaceae;g__Lactococcus` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(ss)
Anova(ss)
summary(glht(ss,linfct=mcp(Ethnicity="Tukey")))

tt = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Turicibacterales;f__Turicibacteraceae;g__Turicibacter` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(tt)
Anova(tt)
summary(glht(tt,linfct=mcp(Ethnicity="Tukey")))

uu = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Clostridiaceae;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(uu)
Anova(uu)
summary(glht(uu,linfct=mcp(Ethnicity="Tukey")))

vv = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Lactobacillales;f__Leuconostocaceae;g__Leuconostoc` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_13year, family = nbinom2)
summary(vv)
Anova(vv)
summary(glht(vv,linfct=mcp(Ethnicity="Tukey")))
```

#####Genera 3-6 years
```{r genera_abund_36years}
genus_meta_36year = genus_meta %>% 
  filter(Host_age_cat == "3-6 years") %>% 
  as.data.frame()

genus_meta_36year$Race = as.factor(genus_meta_36year$Race)

genus_meta_36year$Ethnicity = as.factor(genus_meta_36year$Ethnicity)
...
for(i in 46:1203) {
  variable = genus_meta_36year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = genus_meta_36year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(genus_meta_36year)[i]
  race_pvaluematrix[i-45,1] = names(genus_meta_36year)[i]
  ethnicity_estimatematrix[i-45,1] = names(genus_meta_36year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(genus_meta_36year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_genus_36year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_genus_36year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_genus_36year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_genus_36year.csv")

a = glmmTMB(`k__Bacteria;p__Chloroflexi;c__Anaerolineae;o__S0208;f__;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_36year, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))
...
#####Genera 6-12 years
```{r genera_abund_612years}
genus_meta_612year = genus_meta %>% 
  filter(Host_age_cat == "6-12 years") %>% 
  as.data.frame()

genus_meta_612year$Race = as.factor(genus_meta_612year$Race)

genus_meta_612year$Ethnicity = as.factor(genus_meta_612year$Ethnicity)
...
for(i in 46:1203) {
  variable = genus_meta_612year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = genus_meta_612year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(genus_meta_612year)[i]
  race_pvaluematrix[i-45,1] = names(genus_meta_612year)[i]
  ethnicity_estimatematrix[i-45,1] = names(genus_meta_612year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(genus_meta_612year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_genus_612year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_genus_612year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_genus_612year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_genus_612year.csv")

a = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Bacteroides` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_612year, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))

b = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Porphyromonadaceae;g__Porphyromonas` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_612year, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Race="Tukey")))

c = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Pasteurellales;f__Pasteurellaceae;g__Haemophilus` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_612year, family = nbinom2)
summary(c)
Anova(c)
summary(glht(c,linfct=mcp(Race="Tukey")))

d = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Rikenellaceae;g__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_612year, family = nbinom2)
summary(d)
Anova(d)
summary(glht(d,linfct=mcp(Ethnicity="Tukey")))

e = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;__;__` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_612year, family = nbinom2)
summary(e)
Anova(e)
summary(glht(e,linfct=mcp(Ethnicity="Tukey")))

f = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Deltaproteobacteria;o__Desulfovibrionales;f__Desulfovibrionaceae;g__Desulfovibrio` ~ Race + Ethnicity + (1|Study), 
            data = genus_meta_612year, family = nbinom2)
summary(f)
Anova(f)
summary(glht(f,linfct=mcp(Ethnicity="Tukey")))
...
#####Species 0-1 weeks
```{r species_abund_01week}
species = read_tsv("species.txt")
species_ra = species %>% column_to_rownames("SampleID") %>% 
  decostand(method = "total", MARGIN = 1) %>% rownames_to_column()
species_meta = metadata_re %>% 
  inner_join(species_ra, by = c("SampleID" = "rowname"))
species_meta_01week = species_meta %>% 
  filter(Host_age_cat == "0-1 week") %>% 
  as.data.frame()

species_meta_01week$Race = as.factor(species_meta_01week$Race)

species_meta_01week$Ethnicity = as.factor(species_meta_01week$Ethnicity)

race_estimatematrix = mat.or.vec(1489,2)
race_pvaluematrix = mat.or.vec(1489,2)
ethnicity_estimatematrix = mat.or.vec(1489,2)
ethnicity_pvaluematrix = mat.or.vec(1489,2)

for(i in 46:1534) {
  variable = species_meta_01week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = species_meta_01week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(species_meta_01week)[i]
  race_pvaluematrix[i-45,1] = names(species_meta_01week)[i]
  ethnicity_estimatematrix[i-45,1] = names(species_meta_01week)[i]
  ethnicity_pvaluematrix[i-45,1] = names(species_meta_01week)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_species_01week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_species_01week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_species_01week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_species_01week.csv")

a = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Rikenellaceae;g__Alistipes;s__finegoldii` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_01week, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))

b = glmmTMB(`k__Bacteria;p__Firmicutes;c__Bacilli;o__Bacillales;f__Alicyclobacillaceae;g__Alicyclobacillus;s__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_01week, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Ethnicity="Tukey")))
...
#####Species 1-6 weeks
```{r species_abund_16week}
species_meta = metadata_re %>% 
  inner_join(species_ra, by = c("SampleID" = "rowname"))
species_meta_16week = species_meta %>% 
  filter(Host_age_cat == "1-6 weeks") %>% 
  as.data.frame()

species_meta_16week$Race = as.factor(species_meta_16week$Race)

species_meta_16week$Ethnicity = as.factor(species_meta_16week$Ethnicity)
...
for(i in 46:1534) {
  variable = species_meta_16week[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = species_meta_16week, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(species_meta_16week)[i]
  race_pvaluematrix[i-45,1] = names(species_meta_16week)[i]
  ethnicity_estimatematrix[i-45,1] = names(species_meta_16week)[i]
  ethnicity_pvaluematrix[i-45,1] = names(species_meta_16week)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_species_16week.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_species_16week.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_species_16week.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_species_16week.csv")

a = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Lachnospiraceae;g__Epulopiscium;s__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_16week, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))

b = glmmTMB(`k__Bacteria;p__Actinobacteria;c__Actinobacteria;o__Actinomycetales;f__Actinomycetaceae;g__Actinobaculum;s__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_16week, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Ethnicity="Tukey")))
...
#####Species 6 weeks-3 months
```{r species_abund_6week3months}
species_meta = metadata_re %>% 
  inner_join(species_ra, by = c("SampleID" = "rowname"))
species_meta_6week3month = species_meta %>% 
  filter(Host_age_cat == "6 weeks-3 months") %>% 
  as.data.frame()

species_meta_6week3month$Race = as.factor(species_meta_6week3month$Race)

species_meta_6week3month$Ethnicity = as.factor(species_meta_6week3month$Ethnicity)
...
for(i in 46:1534) {
  variable = species_meta_6week3month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = species_meta_6week3month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(species_meta_6week3month)[i]
  race_pvaluematrix[i-45,1] = names(species_meta_6week3month)[i]
  ethnicity_estimatematrix[i-45,1] = names(species_meta_6week3month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(species_meta_6week3month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_species_6week3month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_species_6week3month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_species_6week3month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_species_6week3month.csv")
```

#####Species 3-12 months
```{r species_abund_312months}
species_meta = metadata_re %>% 
  inner_join(species_ra, by = c("SampleID" = "rowname"))
species_meta_312month = species_meta %>% 
  filter(Host_age_cat == "3-12 months") %>% 
  as.data.frame()

species_meta_312month$Race = as.factor(species_meta_312month$Race)

species_meta_312month$Ethnicity = as.factor(species_meta_312month$Ethnicity)
...
for(i in 46:1534) {
  variable = species_meta_312month[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = species_meta_312month, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(species_meta_312month)[i]
  race_pvaluematrix[i-45,1] = names(species_meta_312month)[i]
  ethnicity_estimatematrix[i-45,1] = names(species_meta_312month)[i]
  ethnicity_pvaluematrix[i-45,1] = names(species_meta_312month)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_species_312month.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_species_312month.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_species_312month.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_species_312month.csv")

a = glmmTMB(`k__Bacteria;p__OD1;c__;o__;f__;g__;s__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_312month, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))

b = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Clostridiaceae;g__;s__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_312month, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Race="Tukey")))

c = glmmTMB(`k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Bacteroidaceae;g__Bacteroides;s__fragilis` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_312month, family = nbinom2)
summary(c)
Anova(c)
summary(glht(c,linfct=mcp(Race="Tukey")))
...
#####Species 1-3 years
```{r species_abund_13years}
species_meta = metadata_re %>% 
  inner_join(species_ra, by = c("SampleID" = "rowname"))
species_meta_13year = species_meta %>% 
  filter(Host_age_cat == "1-3 years") %>% 
  as.data.frame()

species_meta_13year$Race = as.factor(species_meta_13year$Race)

species_meta_13year$Ethnicity = as.factor(species_meta_13year$Ethnicity)
...
for(i in 46:1534) {
  variable = species_meta_13year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = species_meta_13year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(species_meta_13year)[i]
  race_pvaluematrix[i-45,1] = names(species_meta_13year)[i]
  ethnicity_estimatematrix[i-45,1] = names(species_meta_13year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(species_meta_13year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_species_13year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_species_13year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_species_13year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_species_13year.csv")

a = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Peptococcaceae;g__Peptococcus;s__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_13year, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Ethnicity="Tukey")))

b = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Lachnospiraceae;g__Coprococcus;__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_13year, family = nbinom2)
summary(b)
Anova(b)
summary(glht(b,linfct=mcp(Ethnicity="Tukey")))
...
#####Species 3-6 years
```{r species_abund_36year}
species_meta = metadata_re %>% 
  inner_join(species_ra, by = c("SampleID" = "rowname"))
species_meta_36year = species_meta %>% 
  filter(Host_age_cat == "3-6 years") %>% 
  as.data.frame()

species_meta_36year$Race = as.factor(species_meta_36year$Race)

species_meta_36year$Ethnicity = as.factor(species_meta_36year$Ethnicity)
...
for(i in 46:1534) {
  variable = species_meta_36year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = species_meta_36year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(species_meta_36year)[i]
  race_pvaluematrix[i-45,1] = names(species_meta_36year)[i]
  ethnicity_estimatematrix[i-45,1] = names(species_meta_36year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(species_meta_36year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_species_36year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_species_36year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_species_36year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_species_36year.csv")

a = glmmTMB(`k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Ruminococcaceae;g__Clostridium;s__methylpentosum` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_36year, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))
...
#####Species 6-12 years
```{r species_abund_612year}
species_meta = metadata_re %>% 
  inner_join(species_ra, by = c("SampleID" = "rowname"))
species_meta_612year = species_meta %>% 
  filter(Host_age_cat == "6-12 years") %>% 
  as.data.frame()

species_meta_612year$Race = as.factor(species_meta_612year$Race)

species_meta_612year$Ethnicity = as.factor(species_meta_612year$Ethnicity)
...
for(i in 46:1534) {
  variable = species_meta_612year[,i]
  b = try(glmmTMB(variable ~ Race + Ethnicity + (1|Study), 
                  data = species_meta_612year, family = nbinom2))
  anova = Anova(b)
  race_estimatematrix[i-45,2] = anova[1,1]
  race_pvaluematrix[i-45,2] = anova[1,3]
  ethnicity_estimatematrix[i-45,2] = anova[2,1]
  ethnicity_pvaluematrix[i-45,2] = anova[2,3]
  race_estimatematrix[i-45,1] = names(species_meta_612year)[i]
  race_pvaluematrix[i-45,1] = names(species_meta_612year)[i]
  ethnicity_estimatematrix[i-45,1] = names(species_meta_612year)[i]
  ethnicity_pvaluematrix[i-45,1] = names(species_meta_612year)[i]
}

race = bind_cols(as.data.frame(race_estimatematrix[,1:2]), 
                     as.data.frame(race_pvaluematrix[,2]))
write.csv(race, "race_species_612year.csv")
race_nona = filter(race, race[,3] != "NaN") 
race_nona[,3] = as.numeric(as.character(race_nona[,3]))
race_corrected = bind_cols(race_nona, 
                               as.data.frame(fdrtool(race_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(race_corrected, "race_corrected_species_612year.csv")

ethnicity = bind_cols(as.data.frame(ethnicity_estimatematrix[,1:2]), 
                     as.data.frame(ethnicity_pvaluematrix[,2]))
write.csv(ethnicity, "ethnicity_species_612year.csv")
ethnicity_nona = filter(ethnicity, ethnicity[,3] != "NaN") 
ethnicity_nona[,3] = as.numeric(as.character(ethnicity_nona[,3]))
ethnicity_corrected = bind_cols(ethnicity_nona, 
                               as.data.frame(fdrtool(ethnicity_nona[,3], 
                                                     statistic = "pvalue", plot = F)))
write.csv(ethnicity_corrected, "ethnicity_corrected_species_612year.csv")

a = glmmTMB(`k__Bacteria;p__Proteobacteria;c__Epsilonproteobacteria;o__Campylobacterales;f__Campylobacteraceae;g__Campylobacter;s__` ~ Race + Ethnicity + (1|Study), 
            data = species_meta_612year, family = nbinom2)
summary(a)
Anova(a)
summary(glht(a,linfct=mcp(Race="Tukey")))
...
species_meta = species_ra %>% 
  ...
  inner_join(metadata_re)
write_csv(species_meta, "species_relab_meta.csv")
...
genus_meta = genus_ra %>% 
  ...
  pivot_wider(names_from = "Gensu", values_from = "rel_abund") %>% 
  inner_join(metadata_re)
genus_meta$Race = factor(genus_meta$Race, 
                                    levels = c("White", "Black", 
                                               "Asian/Pacific Islander"))
write_csv(genus_meta, "genus_relab_meta.csv")

plot_Prevotella = ggplot(genus_meta,
                      aes(x = Race, 
...
plot_Ruminococcus = ggplot(genus_meta,
                      aes(x = Race, 
...
plot_Collinsella = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Actinobacteria;c__Coriobacteriia;o__Coriobacteriales;f__Coriobacteriaceae;g__Collinsella`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
plot_Alistipes = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Bacteroidetes;c__Bacteroidia;o__Bacteroidales;f__Rikenellaceae;g__Alistipes`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
plot_Coprococcus = ggplot(genus_meta,
                      aes(x = Race, 
...
plot_Veillonella = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Veillonellaceae;g__Veillonella`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
plot_Clostridium = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Lachnospiraceae;g__Clostridium`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
plot_Haemophilus = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Proteobacteria;c__Gammaproteobacteria;o__Pasteurellales;f__Pasteurellaceae;g__Haemophilus`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
plot_Ruminococcaceae = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Ruminococcaceae;__`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
plot_Dorea = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Lachnospiraceae;g__Dorea`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
plot_Bacteroides = ggplot(genus_meta,
                      aes(x = Race, 
...
plot_Faecalibacterium = ggplot(genus_meta,
                      aes(x = Race, 
                          y = `k__Bacteria;p__Firmicutes;c__Clostridia;o__Clostridiales;f__Ruminococcaceae;g__Faecalibacterium`)) + 
  stat_boxplot(geom = 'errorbar', width = 0, 
               color = "black") +
  geom_boxplot(coef = 0, outlier.shape = NA,
               color = "white",
...
importance = read_tsv("feature_importance_children.tsv")
feature_importance_plot = ggplot(as.data.frame(importance), aes(x = perf_metric, y = reorder(names, perf_metric))) + 
...
child_ml_results = readRDS("AGP_comp/child_ml_eth_results.rds")
...
importance = map_dfr(1:100, get_importance)
write_tsv(importance, "feature_importance_children_ethnicity.tsv")

importance = read_tsv("feature_importance_children_ethnicity.tsv")
feature_importance_plot = ggplot(as.data.frame(importance), aes(x = perf_metric, y = reorder(names, perf_metric))) + 
  stat_summary(fun.data = "median_hilow",
               fun.args = list(conf.int = 0.5))

tiff("feature_importance_children_ethnicity.tiff", width = 10, height = 5, units = "in", res = 300)
feature_importance_plot
dev.off()
...
write_csv(importance_summary, "feature_importance_children_ethnicity_mean.csv")
```
...
get_pooled_sens_spec_wvb = function(model_num, specificity) {
  model = child_ml_results[[model_num]]
  ...
  prob_obs_white = bind_cols(prob, observed = observed) %>% dplyr::select(White, observed) %>% 
    filter(observed == "White" | observed == "Black")
  ...
  sens_spec_white = prob_obs_white %>% arrange(desc(White)) %>% 
    mutate(is_white = (observed == "White"),
           ...
           fpr = fp / (total_white$Black),
           specificity = 1 - fpr) %>% 
    dplyr::select(sensitivity, specificity, fpr)
  map_dfr(specificity, get_sensitivity, sens_spec_white) %>% 
    mutate(Comparison = "White vs. Black")
  
}

get_pooled_sens_spec_wva = function(model_num, specificity) {
  model = child_ml_results[[model_num]]
  prob = predict(model$trained_model, model$test_data, type = "prob")
  observed = model$test_data$outcome
  prob_obs_white = bind_cols(prob, observed = observed) %>% dplyr::select(White, observed) %>% 
    filter(observed == "White" | observed == "AAPI")
  total_white = prob_obs_white %>% count(observed) %>% 
    pivot_wider(names_from = observed, values_from = n)
  sens_spec_white = prob_obs_white %>% arrange(desc(White)) %>% 
    mutate(is_white = (observed == "White"),
           ...
           fp = cumsum(!is_white),
           sensitivity = tp / total_white$White,
           fpr = fp / (total_white$AAPI),
           specificity = 1 - fpr) %>% 
    dplyr::select(sensitivity, specificity, fpr)
  map_dfr(specificity, get_sensitivity, sens_spec_white) %>% 
    mutate(Comparison = "White vs. Asian/Pacific Islander")
...
pooled_sens_spec_wvb = map_dfr(1:100, get_pooled_sens_spec_wvb, specificity)
pooled_sens_spec_wva = map_dfr(1:100, get_pooled_sens_spec_wva, specificity)

pooled_sens_spec = bind_rows(pooled_sens_spec_wvb, pooled_sens_spec_wva)
pooled_sens_spec$Comparison = factor(pooled_sens_spec$Comparison, levels = c("White vs. Black", "White vs. Asian/Pacific Islander"))

saveRDS(pooled_sens_spec, "pooled_sens_spec_r.rds")
pooled_sens_spec = readRDS("pooled_sens_spec_r.rds")
write_csv(pooled_sens_spec, "childonly_rfmodel_roc_curve.csv")

roc_curve_child_r = pooled_sens_spec %>% group_by(Comparison, specificity) %>% 
  ...
  geom_ribbon(alpha=0.25, aes(fill = Comparison)) + 
  geom_step(aes(color = Comparison), size = 0.8) + 
  xlab("False Positive Rate") + ylab("True Positive Rate") + 
  scale_color_manual(values = c("darkorange4", "darkslategray4")) + 
  scale_fill_manual(values = c("darkorange4", "darkslategray4")) +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "AUC = 0.914<br>
           prAUC = 0.629<br>
           Accuracy = 0.879<br>
...
get_pooled_sens_spec_his = function(model_num, specificity) {
  ...
  map_dfr(specificity, get_sensitivity, sens_spec_his) %>% 
    mutate(Comparison = "Hispanic vs. Non-Hispanic")
...
saveRDS(pooled_sens_spec_his, "pooled_sens_spec_e.rds")
pooled_sens_spec = readRDS("pooled_sens_spec_e.rds")

roc_curve_child_e = pooled_sens_spec_his %>% group_by(Comparison, specificity) %>% 
  summarize(lquartile = quantile(sensitivity, prob = 0.25),
            uquartile = quantile(sensitivity, prob = 0.75),
            sensitivity = median(sensitivity),
            .groups = "drop") %>% 
  ...
  geom_ribbon(alpha=0.25, aes(fill = Comparison)) + 
  geom_step(aes(color = Comparison), size = 0.8) + 
  scale_color_manual(values = c("orchid4")) + 
  scale_fill_manual(values = c("orchid4")) + 
  xlab("False Positive Rate") + ylab("True Positive Rate") +
  annotate(geom = "richtext", fill = NA, label.color = NA,
           label = "AUC = 0.886<br>
           prAUC = 0.797<br>
...
importance = read_tsv("feature_importance_merged.tsv")
feature_importance_plot = ggplot(as.data.frame(importance), aes(x = perf_metric, y = reorder(names, perf_metric))) + 
  stat_summary(fun.data = "median_hilow",
               fun.args = list(conf.int = 0.5))
...
pooled_sens_spec = bind_rows(pooled_sens_spec_wva, pooled_sens_spec_wvb)
pooled_sens_spec$Comparison = factor(pooled_sens_spec$Comparison, levels = c("White vs. Black", "White vs. Asian/Pacific Islander"))

saveRDS(pooled_sens_spec, "pooled_sens_spec_adultchild_r.rds")
pooled_sens_spec = readRDS("pooled_sens_spec_adultchild_r.rds")
write_csv(pooled_sens_spec, "childadult_rfmodel_roc_curve.csv")

pooled_sens_spec %>% group_by(Comparison, specificity) %>% 
  summarize(lquartile = quantile(sensitivity, prob = 0.25),
            uquartile = quantile(sensitivity, prob = 0.75),
            sensitivity = median(sensitivity),
            .groups = "drop") %>% 
...
genus_race_merged_down = readRDS("AGP_comp/genus_race_merged_all.rds")

genus_race_merged_down %>%
  map(pluck, "trained_model") %>% 
  combine_hp_performance() %>% 
  pluck("dat") %>% 
  write_tsv("genus_merged_down_ml_pooled_hp.tsv")


lapply(genus_race_merged_down, function(result) {
  result[['performance']] %>% 
    dplyr::select(cv_metric_logLoss, Accuracy, method)
}) %>% 
  bind_rows() %>%  
  write_tsv("genus_merged_down_ml_pooled_performance.tsv")

get_importance = function(model_num) {
  model = genus_race_merged_down[[model_num]]
...
importance = map_dfr(1:1000, get_importance)
write_tsv(importance, "feature_importance_genus_down.tsv")

feature_importance_plot = ggplot(as.data.frame(importance), aes(x = perf_metric, y = names)) + 
  stat_summary(fun.data = "median_hilow",
               fun.args = list(conf.int = 0.5))

tiff("feature_importance_genus_down.tiff", width = 10, height = 10, units = "in", res = 300)
feature_importance_plot
dev.off()

importance_summary = importance %>% 
  dplyr::select(-perf_metric_diff, -pvalue, 
         -method, -perf_metric_name) %>% 
  pivot_wider(id_cols = "names", 
              names_from = "seed", 
              names_prefix = "seed",
              values_from = "perf_metric") %>% 
  mutate(mean_importance = rowMeans(dplyr::select(., starts_with("seed"))))

write_csv(importance_summary, "feature_importance_genus_merged_down_mean.csv")
```
###Roc curves
```{r merge_down_ml_roc}
genus_race_merged_down = readRDS("AGP_comp/genus_race_merged_all.rds")
...
performance = lapply(genus_race_merged_down, function(result) {
  result[['performance']] %>% 
    dplyr::select(AUC, prAUC, Accuracy, Mean_Sensitivity, Mean_Specificity)
}) %>% 
  bind_rows() %>% 
...
pooled_sens_spec_wvb = map_dfr(1:1000, get_pooled_sens_spec_wvb, specificity)
pooled_sens_spec_wva = map_dfr(1:1000, get_pooled_sens_spec_wva, specificity)

pooled_sens_spec = bind_rows(pooled_sens_spec_wva, pooled_sens_spec_wvb)
pooled_sens_spec$Comparison = factor(pooled_sens_spec$Comparison, levels = c("White vs. Black", "White vs. Asian/Pacific Islander"))

saveRDS(pooled_sens_spec, "pooled_sens_spec_adultchild_down_r.rds")
...
ggsave("roc_curve_merge_down.tiff", width = 7, height = 5)
```
...
write_csv(genus_merged_graphs_relab2, "genus_adultchild_meta.csv")
...
tiff(file="fig1.tif", res=300, width=25, height=8, units="in")
...
setEPS()
postscript(file="fig1.eps", width=25, height=8, paper = "special")
fig1
dev.off()
```
...
tiff(file="fig2.tif", res=300, width=22, height=20, units="in")
...
setEPS()
postscript(file="fig2.eps", width=22, height=20, paper = "special")
plot
dev.off()
```
...
venn.diagram(x = list(c, a), 
             category.names = c("Children (57)", "Adults (144)"), 
             filename = "fig3B.tif", output = T, imagetype = "tiff", 
...
venn.diagram(x = list(c, a), 
             category.names = c("Children (57)", "Adults (144)"), 
             filename = "fig3B.svg", output = T, imagetype = "svg", 
             height = 5, width = 5, units = "in", resolution = 300, 
             col = c("#4b265c", "#25a165"), lwd = 1,
             fill = c(alpha("#4b265c", 0.3), alpha("#25a165", 0.3)), 
             fontfamily = "sans", cat.fontfamily = "sans",
...
### Figure 3c
```{r fig3C}
tiff(file="fig3C.tif", res=300, width=7, height=10, units="in")
plot_grid(roc_curve_child_r, roc_curve_child_e, nrow = 2, ncol = 1,
          rel_widths = c(1, 1), align = "hv", axis = "tb")
dev.off()

setEPS()
postscript(file="fig3C.eps", width=7, height=10, paper = "special")
plot_grid(roc_curve_child_r, roc_curve_child_e, nrow = 2, ncol = 1,
          rel_widths = c(1, 1), align = "hv", axis = "tb")
dev.off()
```
...
####Figure S10
Barplots of observed vs. expected values for factors included in models by race.
```{r figs10}
figs23_data = read_tsv("tableS11_barplots_race.txt", col_types = "ffd")

figs23_data_sex = figs23_data %>% 
  filter(Factor == "Female Observed" | Factor == "Female Expected" |
           Factor == "Male Observed" | Factor == "Male Expected")

figs23_sex = ggbarplot(figs23_data_sex, x = "Race", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#4daf4a", "#91cd89", "#ff7f00", "#ffae6d"),
                       position = position_dodge(0.9), legend = "right") +
  rremove("legend.title")

figs23_data_age = figs23_data %>% 
  filter(Factor == "0-0.9 week Observed" | Factor == "0-0.9 week Expected" |
           Factor == "1-5.9 weeks Observed" | Factor == "1-5.9 weeks Expected" |
           Factor == "6 weeks-2.9 months Observed" | Factor == "6 weeks-2.9 months Expected" |
           Factor == "3-11.9 months Observed" | Factor == "3-11.9 months Expected" |
           Factor == "12-35.9 months Observed" | Factor == "12-35.9 months Expected" | Factor == "3-5.9 years Observed" | Factor == "3-5.9 years Expected" |
           Factor == "6-11.9 years Observed" | Factor == "6-11.9 years Expected")

figs23_data_age$Factor = factor(figs23_data_age$Factor, levels = c("0-0.9 week Observed", "0-0.9 week Expected", "1-5.9 weeks Observed", "1-5.9 weeks Expected", "6 weeks-2.9 months Observed", "6 weeks-2.9 months Expected", "3-11.9 months Observed", "3-11.9 months Expected", "12-35.9 months Observed", "12-35.9 months Expected", "3-5.9 years Observed", "3-5.9 years Expected", "6-11.9 years Observed", "6-11.9 years Expected"))

figs23_age = ggbarplot(figs23_data_age, x = "Race", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#bc80bd", "#f9d9f8", "#8dd3c7", 
                                   "#b5eee3", "#bebada", 
                                   "#e3deff", "#fb8072", "#ffd7cf",
                                   "#80b1d3", "#bfe8ff", "#fdb462", 
                                   "#fdddbf", "#b3de69", "#dbe7c2"),
                       position = position_dodge(0.9), legend = "right") +
  rremove("legend.title")

figs23_data_del = figs23_data %>% 
  filter(Factor == "Cesarean Observed" | Factor == "Cesarean Expected" |
           Factor == "Vaginal Observed" | Factor == "Vaginal Expected")

figs23_del = ggbarplot(figs23_data_del, x = "Race", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#8da0cb", "#d4e2ff", "#a6d854", "#dbe7c2"),
                       position = position_dodge(0.9), legend = "right") +
  rremove("legend.title")

figs23_data_fed = figs23_data %>% 
  filter(Factor == "Exclusively Breastfed Observed" | Factor == "Exclusively Breastfed Expected" |
           Factor == "Formula Feeding Observed" | Factor == "Formula Feeding Expected" |
           Factor == "Mixed Feeding Observed" | Factor == "Mixed Feeding Expected")

figs23_fed = ggbarplot(figs23_data_fed, x = "Race", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#66c2a6", "#baeddb", "#fc8d62", "#ffd9c7", "#b3b3b3", "#e2e2e2"),
                       position = position_dodge(0.9), legend = "right") +
  rremove("legend.title")

setEPS()
postscript(file="figS10.eps", width=15, height=9, paper = "special")
ggarrange(figs23_age, figs23_sex, figs23_del, figs23_fed, 
          nrow = 2, ncol = 2, align = "hv")
dev.off()
```

####Figure S11
Barplots of observed vs. expected values for factors included in models by ethnicity
```{r figs11}
figs24_data = read_tsv("tableS11_barplots_ethnicity.txt", col_types = "ffd")

figs24_data_sex = figs24_data %>% 
  filter(Factor == "Female Observed" | Factor == "Female Expected" |
           Factor == "Male Observed" | Factor == "Male Expected")

figs24_sex = ggbarplot(figs24_data_sex, x = "Ethnicity", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#4daf4a", "#91cd89", "#ff7f00", "#ffae6d"),
                       position = position_dodge(0.9), legend = "right") +
  rremove("legend.title")

figs24_data_age = figs24_data %>% 
  filter(Factor == "0-0.9 week Observed" | Factor == "0-0.9 week Expected" |
           Factor == "1-5.9 weeks Observed" | Factor == "1-5.9 weeks Expected" |
           Factor == "6 weeks-2.9 months Observed" | Factor == "6 weeks-2.9 months Expected" |
           Factor == "3-11.9 months Observed" | Factor == "3-11.9 months Expected" |
...
figs24_data_age$Factor = factor(figs24_data_age$Factor, levels = c("0-0.9 week Observed", "0-0.9 week Expected", "1-5.9 weeks Observed", "1-5.9 weeks Expected", "6 weeks-2.9 months Observed", "6 weeks-2.9 months Expected", "3-11.9 months Observed", "3-11.9 months Expected", "12-35.9 months Observed", "12-35.9 months Expected", "3-5.9 years Observed", "3-5.9 years Expected", "6-11.9 years Observed", "6-11.9 years Expected"))

figs24_age = ggbarplot(figs24_data_age, x = "Ethnicity", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#bc80bd", "#f9d9f8", "#8dd3c7", 
                                   "#b5eee3", "#bebada", 
                                   "#e3deff", "#fb8072", "#ffd7cf",
...
figs24_data_del = figs24_data %>% 
  filter(Factor == "Cesarean Observed" | Factor == "Cesarean Expected" |
           Factor == "Vaginal Observed" | Factor == "Vaginal Expected")

figs24_del = ggbarplot(figs24_data_del, x = "Ethnicity", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#8da0cb", "#d4e2ff", "#a6d854", "#dbe7c2"),
                       position = position_dodge(0.9), legend = "right") +
  rremove("legend.title")

figs24_data_fed = figs24_data %>% 
  filter(Factor == "Exclusively Breastfed Observed" | Factor == "Exclusively Breastfed Expected" |
           Factor == "Formula Feeding Observed" | Factor == "Formula Feeding Expected" |
           Factor == "Mixed Feeding Observed" | Factor == "Mixed Feeding Expected")

figs24_fed = ggbarplot(figs24_data_fed, x = "Ethnicity", y = "Individuals",
                       fill = "Factor", 
                       palette = c("#66c2a6", "#baeddb", "#fc8d62", "#ffd9c7", "#b3b3b3", "#e2e2e2"),
                       position = position_dodge(0.9), legend = "right") +
  rremove("legend.title")
...
setEPS()
postscript(file="figs11.eps", width=12, height=9, paper = "special")
ggarrange(figs24_age, figs24_sex, figs24_del, figs24_fed, 
          nrow = 2, ncol = 2, align = "hv")
dev.off()
```
#### Figure S12
...
tiff(file="figs12.tif", res=300, width=12, height=24, units="in")
...
setEPS()
postscript(file="figs12.eps", width=12, height=24, paper = "special")
plot
dev.off()
```
...
tiff(file="figs13.tif", res=300, width=18, height=24, units="in")
...
setEPS()
postscript(file="figs13.eps", width=18, height=24, paper = "special")
plot
dev.off()
```

####Figure S14
Correlation plots of taxa that were differentially abundant across ages (from ANCOMBC results) and differed in abundance between racial categories EITHER in the overall results OR in specific age categories.
```{r dif_abund_age}
species_meta_clean = clean_names(species_meta)
species_meta_clean$race = factor(species_meta_clean$race, levels = c("White", "Black", "Asian/Pacific Islander"))

r1plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_od1_c_o_f_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "OD1 spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r2plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Ruminococcaceae spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r3plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_dorea_s_longicatena", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Dorea longicatena", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r4plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Lachnospiraceae spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r5plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridiales spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r6plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridiales spp. 2", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r7plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_veillonella_s_dispar", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Veillonella dispar", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r8plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides_s_caccae", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Bacteroides caccae", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r9plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_prevotellaceae_g_prevotella_s_copri", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Prevotella copri", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r10plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_ruminococcus_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Ruminococcus spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r11plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_tissierellaceae_g_peptoniphilus_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Peptoniphilus spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r12plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Bacteroides spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r13plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_blautia_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Blautia spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r14plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_coprococcus_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Coprococcus spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r15plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Bacteroides spp. 2", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r16plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_erysipelotrichi_o_erysipelotrichales_f_erysipelotrichaceae_g_clostridium_s_spiroforme", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridium spiroforme", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r17plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_dialister_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Dialister spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r18plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_roseburia_s_faecis", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Roseburia faecis", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r19plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_porphyromonadaceae_g_parabacteroides_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Parabacteroides spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r20plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_dorea_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Dorea spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r21plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_coriobacteriia_o_coriobacteriales_f_coriobacteriaceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Coriobacteriaceae spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r22plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_proteobacteria_c_alphaproteobacteria_o_rf32_f_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "RF32 spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r23plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli_o_bacillales_f_staphylococcaceae_g_staphylococcus", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Staphylococcus spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r24plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_megasphaera_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Megasphaera spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r25plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_ruminococcus_s_gnavus", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "[Ruminococcus] gnavus", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r26plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides_s_fragilis", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Bacteroides fragilis", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r27plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_fusobacteria_c_fusobacteriia_o_fusobacteriales_f_fusobacteriaceae_g_fusobacterium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Fusobacterium spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r28plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Ruminococcaceae spp. 2", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r29plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_od1", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "OD1 spp. 2", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r30plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_ruminococcus_s_bromii", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Ruminococcus bromii", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r31plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "unassigned", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Unassigned", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r32plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_veillonella_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Veillonella spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r33plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_ruminococcus_s_torques", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "[Ruminococcus] torques", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r34plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_faecalibacterium", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Faecalibacterium spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r35plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_eubacteriaceae_g_pseudoramibacter_eubacterium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Pseudoramibacter spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r36plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_peptostreptococcaceae", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Peptostreptococcaceae spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r37plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_erysipelotrichi_o_erysipelotrichales_f_erysipelotrichaceae_g_clostridium_s_ramosum", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridium ramosum", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r38plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_rikenellaceae_g_alistipes_s_finegoldii", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Alistipes finegoldii", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r39plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides_s_ovatus", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Bacteroides ovatus", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r40plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_rikenellaceae_g_alistipes_s_onderdonkii", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Alistipes onderdonkii", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r41plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_coriobacteriia_o_coriobacteriales_f_coriobacteriaceae_g_collinsella_s_aerofaciens", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Collinsella aerofaciens", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r42plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_rikenellaceae_g_alistipes_s_putredinis", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Alistipes putredinis", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r43plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_proteobacteria_c_epsilonproteobacteria_o_campylobacterales_f_campylobacteraceae_g_campylobacter_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Campylobacter spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r44plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_clostridium", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridium spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r45plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_gemmiger_s_formicilis", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Gemmiger formicilis", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r47plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_epulopiscium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Epulopiscium", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r48plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_clostridium_s_lavalense", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridium lavalense", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r50plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_coriobacteriia_o_coriobacteriales_f_coriobacteriaceae_g_eggerthella_s_lenta", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Eggerthella lenta", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r51plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_veillonella_s_parvula", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Veillonella parvula", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r52plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Lachnospiraceae spp. 2", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r53plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_odoribacteraceae_g_odoribacter_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Odoribacter spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r54plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli_o_lactobacillales_f_streptococcaceae_g_lactococcus_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Lactococus spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r55plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_dorea_s_formicigenerans", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Dorea formicigenerans", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r56plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Bacilli spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r57plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_lachnobacterium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Lachnobacterium spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r58plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_anaerostipes_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Anaerostipes spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r59plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli_o_lactobacillales_f_lactobacillaceae_g_lactobacillus_s_zeae", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Lactobacillus zeae", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r60plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_clostridiaceae_g_clostridium_s_paraputrificum", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridium paraputrificum", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r61plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_clostridium_s_aldenense", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridium aldenense", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r62plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_clostridiaceae_g_clostridium_s_neonatale", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Clostridium neonatale", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r63plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_proteobacteria_c_gammaproteobacteria_o_pasteurellales_f_pasteurellaceae_g_haemophilus", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Haemophilus", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

r64plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_actinobacteria_o_bifidobacteriales_f_bifidobacteriaceae_g_bifidobacterium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "race", shape = 1, 
          palette = c("#377eb8", "#f6d128", "#e41a1c"), 
          ylab = "Bifidobacterium", xlab = "Age (years)") + 
  stat_cor(aes(color = race), label.x = 8, 
           label.y.npc = "top", size = 3)

setEPS()
postscript(file="figs14a.eps", width=18, height=22, paper = "special")
ggarrange(r1plot, r2plot, r3plot, r4plot, r5plot, r6plot, r7plot, r8plot,
          r9plot, r10plot, r11plot, r12plot, r13plot, r14plot, r15plot,
          r16plot, r17plot, r18plot, r19plot, r20plot, r21plot, r22plot,
          r23plot, r24plot, r25plot, r26plot, r27plot, r28plot, r29plot,
          r30plot, r31plot, r32plot, r33plot, r34plot, r35plot, 
          nrow = 7, ncol = 5,
          align = "hv", common.legend = T, legend = "bottom")
dev.off()

setEPS()
postscript(file="figs14a.eps", width=18, height=19, paper = "special")
ggarrange(r36plot,
          r37plot, r38plot, r39plot, r40plot, r41plot, r42plot, r43plot,
          r44plot, r45plot, r47plot, r48plot, r50plot, r51plot, r52plot,
          r53plot, r54plot, r55plot, r56plot, r57plot, r58plot, r59plot,
          r60plot, r61plot, r62plot, r63plot, r64plot, nrow = 6, ncol = 5,
          align = "hv", common.legend = T, legend = "bottom")
dev.off()
```

####Figure S15
Correlation plots of taxa that were differentially abundant across ages (from ANCOMBC results) and differed in abundance between ethnic categories EITHER in the overall results OR in specific age categories.
```{r plot_s15}

species_meta_clean = clean_names(species_meta)
species_meta_clean$ethnicity = factor(species_meta_clean$ethnicity, levels = c("Hispanic", "Non Hispanic"))

e1plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_dorea_s_longicatena", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Dorea longicatena", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e2plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_erysipelotrichi_o_erysipelotrichales_f_erysipelotrichaceae_g_eubacterium_s_dolichum", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "[Eubacterium] dolichum", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e3plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_clostridiaceae_g_clostridium_s_perfringens", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Clostridium perfringens", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e4plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e5plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_proteobacteria_c_gammaproteobacteria_o_enterobacteriales_f_enterobacteriaceae", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Enterobacteriaceae spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e6plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_erysipelotrichi_o_erysipelotrichales_f_erysipelotrichaceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Erysipelotrichaceae spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e7plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli_o_lactobacillales_f_streptococcaceae_g_streptococcus_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Streptococcus spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e8plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_faecalibacterium_s_prausnitzii", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Faecalibacterium prausnitzii", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e9plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e10plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_veillonella_s_dispar", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e11plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides_s_caccae", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e12plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_prevotellaceae_g_prevotella_s_copri", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e13plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e14plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_bacteroidaceae_g_bacteroides_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e15plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_actinobacteria_o_bifidobacteriales_f_bifidobacteriaceae_g_bifidobacterium", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Bifidobacterium spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e16plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_dialister_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e17plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_dorea_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e18plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_coriobacteriia_o_coriobacteriales_f_coriobacteriaceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e19plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_roseburia", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Roseburia spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e20plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_christensenellaceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Christensenellaceae spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e21plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_actinobacteria_o_actinomycetales_f_corynebacteriaceae_g_corynebacterium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Corynebacterium spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e22plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_clostridium_s_hathewayi", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Clostridium hathewayi", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e23plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_ruminococcus_s_gnavus", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e24plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e25plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli_o_lactobacillales_f_streptococcaceae_g_streptococcus", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Streptococcus spp.", xlab = "Age (years)") + 
...
e26plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_od1", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e27plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_barnesiellaceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "[Barnesiellaceae] spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)
...
e28plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "unassigned", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e29plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_actinobacteria_o_bifidobacteriales_f_bifidobacteriaceae_g_bifidobacterium_s_bifidum", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Bifidobacterium bifidum", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e30plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_veillonellaceae_g_veillonella_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e31plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_faecalibacterium", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e32plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_clostridium_s_citroniae", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Clostridium citroniae", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e33plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_erysipelotrichi_o_erysipelotrichales_f_erysipelotrichaceae_g_clostridium_s_ramosum", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e34plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_rikenellaceae_g_alistipes_s_onderdonkii", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e35plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_coriobacteriia_o_coriobacteriales_f_coriobacteriaceae_g_collinsella_s_aerofaciens", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e36plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_bacteroidetes_c_bacteroidia_o_bacteroidales_f_rikenellaceae_g_alistipes_s_putredinis", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e37plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_gemmiger_s_formicilis", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e38plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_epulopiscium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Epulopiscium spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e39plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_ruminococcus_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "[Ruminococcus] spp.", xlab = "Age (years)") + 
...
e40plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_actinobacteria_o_actinomycetales_f_actinomycetaceae_g_actinomyces_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Actinomyces spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e41plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_clostridium_s_lavalense", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e42plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_actinobacteria_c_coriobacteriia_o_coriobacteriales_f_coriobacteriaceae_g_eggerthella_s_lenta", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e43plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e44plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_ruminococcaceae_g_ruminococcus", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Ruminococcus spp.", xlab = "Age (years)") + 
...
e45plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli_o_lactobacillales_f_streptococcaceae_g_lactococcus_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Lactococcus spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e46plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_bacilli_o_turicibacterales_f_turicibacteraceae_g_turicibacter_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Turicibacter spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e47plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_dorea_s_formicigenerans", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e48plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Clostridia spp.", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e49plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_blautia_s_obeum", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Blautia obeum", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e50plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_lachnobacterium_s", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e51plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_coprococcus_s_eutactus", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Coprococcus eutactus", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

e52plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_clostridiaceae_g_clostridium_s_neonatale", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
...
e53plot = ggscatter(species_meta_clean, x = "host_age_years", 
          y = "k_bacteria_p_firmicutes_c_clostridia_o_clostridiales_f_lachnospiraceae_g_clostridium_s_symbiosum", 
          add = "reg.line", conf.int = TRUE, 
          color = "ethnicity", shape = 1, 
          palette = c("#4b265c", "#25a165"), 
          ylab = "Clostridium symbiosum", xlab = "Age (years)") + 
  stat_cor(aes(color = ethnicity), label.x = 8, 
           label.y.npc = "top", size = 3)

setEPS()
postscript(file="figs15a.eps", width=18, height=20, paper = "special")
ggarrange(e1plot, e2plot, e3plot, e4plot, e5plot, e6plot, e7plot, e8plot,
          e9plot, e10plot, e11plot, e12plot, e13plot, e14plot, e15plot,
          e16plot, e17plot, e18plot, e19plot, e20plot, e21plot, e22plot,
          e23plot, e24plot, e25plot, e26plot, e27plot, e28plot, e29plot,
          e30plot, nrow = 6, ncol = 5,
          align = "hv", common.legend = T, legend = "bottom")
dev.off()

setEPS()
postscript(file="figs15b.eps", width=18, height=20, paper = "special")
ggarrange(e31plot, e32plot, e33plot, e34plot, e35plot, e36plot,
          e37plot, e38plot, e39plot, e40plot, e41plot, e42plot, e43plot,
          e44plot, e45plot, e47plot, e48plot, e50plot, e51plot, e52plot,
          e53plot, nrow = 5, ncol = 5,
          align = "hv", common.legend = T, legend = "bottom")
dev.off()
```
...
#### Figure S17
```{r figs17}
tiff(file="figS17.tif", 
     res=300, width=20, height=16, units="in")
plots15 = ggarrange(plot_Alistipes, plot_Bacteroides, plot_Blautia, plot_Clostridium, plot_Collinsella, plot_Coprococcus, plot_Dorea, plot_Faecalibacterium, plot_Haemophilus, plot_Prevotella, plot_Ruminococcaceae, plot_Ruminococcus, plot_Veillonella,
          nrow = 3, ncol = 5, common.legend = T, legend = "right")
dev.off()

setEPS()
postscript(file="figs17.eps", width=20, height=16, paper = "special")
plots15
dev.off()
```
...
tiff(file="figs20.tif", res=300, width=20, height=6, units="in")
...
setEPS()
postscript(file="figs20.eps", width=20, height=6, paper = "special")
plot
dev.off()
```
...
#### Figure S21
```{r figure_s21}
tiff(file="figS21.tif", 
     res=300, width=10, height=6, units="in")
seq_depth_plot
dev.off()

setEPS()
postscript(file="figs21.eps", width=10, height=6, paper = "special")
seq_depth_plot
dev.off()
```
